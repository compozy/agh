...
describe("fetchHealth", () => {
  ...
  it("returns parsed HealthPayload on success", async () => {
    ...
    expect(result).toEqual(statusFixture.health);
  ...
  it("throws on network error (non-ok response)", async () => {
    ...
    await expect(fetchHealth()).rejects.toThrow("Runtime status check failed: 500");
  });
...
describe("fetchDaemonStatus", () => {
  ...
  it("returns parsed DaemonStatusPayload on success", async () => {
    ...
    expect(result).toEqual(statusFixture.daemon);
  ...
  it("throws on network error (non-ok response)", async () => {
    ...
    await expect(fetchDaemonStatus()).rejects.toThrow("Runtime status check failed: 500");
  });
