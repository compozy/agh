...
function filterMemories(memories: MemoryHeader[], query: string): MemoryHeader[] {
...
function groupMemoriesByScope(memories: MemoryHeader[]): MemoryGroup[] {
  ...
  return Array.from(buckets.entries())
    ...
    .map(([scope, items]) => ({
      ...
      memories: items.slice().sort((a, b) => a.name.localeCompare(b.name)),
    }));
}
...
function KnowledgeListPanel({
  ...
  const filtered = useMemo(() => filterMemories(memories, searchQuery), [memories, searchQuery]);
