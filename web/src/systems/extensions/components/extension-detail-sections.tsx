...
function TokenBlock({ label, values }: { label: string; values: string[] }) {
  return (
    <DetailBlock label={`${label} ${values.length}`}>
      {values.length ? (
...
function EnvironmentState({ required, missing }: { required: string[]; missing: string[] }) {
  if (!required.length)
          ...
          provided.map(bundle => {
            ...
            return (
              <div className="flex items-center justify-between gap-3 px-4 py-3" key={bundle.name}>
                ...
                {activation ? (
                  <Button
                    render={<Link params={{ id: activation.id }} to="/extensions/bundles/$id" />}
                    nativeButton={false}
                    size="sm"
                    variant="ghost"
                  >
