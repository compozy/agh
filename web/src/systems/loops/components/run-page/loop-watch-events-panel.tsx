    ...
    <div className="border-b border-line-soft px-4 py-4" data-testid="loop-watch-events-panel">
      ...
      <div className="flex flex-col gap-2" data-testid="loop-watch-events-subscriptions">
        {subscriptions.map(({ item: subscription, key }) => (
          ...
          >
            <span className="block font-mono text-mono-id font-medium text-fg-strong">
              {subscription.kind}
            </span>
            {subscription.filter ? (
              <span
                className="mt-1 block font-mono text-mono-id leading-relaxed break-words text-muted"
                data-testid="loop-watch-events-filter"
              >
                {subscription.filter}
              </span>
            ) : (
              <span className="mt-1 block text-badge text-faint">matches every event</span>
            )}
          </div>
        ))}
      </div>
      {cursors.length > 0 ? (
        <div className="mt-3" data-testid="loop-watch-events-cursors">
          <span className="mb-1 block text-badge text-faint">Cursors</span>
          {cursors.map(([stream, seq]) => (
            <div
              key={stream}
              className="flex items-baseline justify-between gap-3 py-0.5 font-mono text-mono-id"
            >
      ...
      {state.last_wake_at ? (
        <div className="mt-3 flex items-baseline justify-between gap-3 text-form-hint">
