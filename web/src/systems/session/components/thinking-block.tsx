 ...
 * icon well + `size-3.5` Brain glyph, an italic label, an optional "N updates"
      ...
      <Collapsible open={open} onOpenChange={setUserOpen}>
        <CollapsibleTrigger
          className={cn(
            ...
            "rounded-sm px-1 text-left text-small-body",
        ...
        >
          <span className="flex size-5 shrink-0 items-center justify-center">
            <Brain
              className="size-3.5 shrink-0 text-faint transition-colors group-hover/reasoning:text-muted"
              strokeWidth={1.75}
              aria-hidden="true"
            />
          </span>
          <span className="flex min-w-0 flex-1 items-center gap-1.5">
            <span className="truncate italic text-subtle transition-colors group-hover/reasoning:text-muted">
              {label}
            </span>
          ...
          <span className="flex shrink-0 items-center gap-1.5 text-subtle">
            {updateCount > 1 ? (
              <Eyebrow className="text-subtle">{updateCount} updates</Eyebrow>
            ) : null}
            <ChevronRight
              className={cn(
                "size-3 shrink-0 text-faint transition-transform duration-base ease-out",
