...
export function StorySurface({ children, className }: StoryFrameProps) {
  return <div className={cn("min-h-[640px] bg-background p-6 text-fg", className)}>{children}</div>;
}
...
export function PanelSurface({ children, className }: StoryFrameProps) {
  return (
    <StorySurface
      className={cn(
        "flex min-h-[640px] overflow-hidden rounded-2xl border border-line bg-canvas p-0",
        className
      )}
    >
      {children}
