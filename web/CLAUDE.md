...
- **Flat depth only**: no freehand `box-shadow`, no content gradients, no glass. Depth = warm surface ramp + `--color-line*` hairlines + exported `--shadow-*` tokens. Sole carve-out: OS-shell chrome (menubar, dock, sessions rail, shell popovers, window frames) uses the tokenized shell glass/blur/window shadows — window-body content stays flat (DESIGN.md §5).
