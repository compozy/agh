...
## Subtasks
- [x] 17.1 Extend detail-route orchestration with task-tree live state and view-mode control
- [x] 17.2 Implement the multi-agent live panel/tree components for parent and descendant execution state
- [x] 17.3 Add linked-session and active-run affordances without regressing route-state clarity
- [x] 17.4 Add tests for tree rendering, live updates, and fallback states
...
## Tests
- Unit tests:
  - [x] Multi-agent live components render parent and descendant task state from the tree live read with stable grouping and hierarchy cues
  - [x] Live-state components handle no-active-run, loading, disconnected, and no-descendant states gracefully
  - [x] Descendant status chips, active-run badges, and latest-activity summaries remain stable as tree data changes
  - [x] Linked-session affordances resolve the correct descendant run and session context for drill-down actions
  - [x] Route-level live mode selection remains stable as tree data changes or refreshes occur
- Integration tests:
  - [x] The detail route can switch into the multi-agent live view without refetch storms or recursive detail joins
  - [x] Task-tree live updates refresh the visible descendant state coherently as parent and child execution changes arrive
  - [x] Navigation from the live tree into run detail or session drill-down preserves route state correctly
  - [x] The live view falls back cleanly when the stream disconnects or the tree read returns no active descendants
