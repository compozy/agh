...
## Subtasks
- [ ] 1.1 Add `AutomationConfig` and nested TOML structs to `internal/config`
- [ ] 1.2 Create the foundational `internal/automation/` package with the core domain structs and enums
- [ ] 1.3 Add validation helpers for scope, schedule mode, webhook fields, retry policy, and fire-limit policy
- [ ] 1.4 Add template-parse validation entry points for trigger prompts using the strict activation-envelope model
- [ ] 1.5 Cover config loading and type validation with table-driven tests
...
## Tests
- Unit tests:
  - [ ] Parsing TOML with one global job and one workspace trigger populates the expected `scope`, `workspace`, and source fields
  - [ ] Validation rejects `scope = "global"` definitions that also provide a workspace binding
  - [ ] Validation rejects `scope = "workspace"` definitions that omit a workspace binding
  - [ ] Validation rejects webhook-only fields on non-webhook triggers and missing webhook fields on webhook triggers
  - [ ] Validation rejects unsupported schedule modes, malformed retry settings, and malformed fire-limit windows
- Integration tests:
  - [ ] Loading a global home config plus a workspace overlay preserves automation defaults and applies workspace-specific automation entries without breaking unrelated config sections
  - [ ] Trigger prompt validation fails fast for a template that references a missing activation-envelope field under `missingkey=error`
