...
- Decision: `invalid`
- Notes:
  - `internal/api/core/conversions.go` no longer serializes raw `WebhookSecretRef`; it delegates to `contract.TriggerPayloadFromTrigger`, which emits only `webhook_secret_present` and `webhook_secret_hash`.
  - The cited leak path is already removed in the current implementation, so no additional production change is needed here.
