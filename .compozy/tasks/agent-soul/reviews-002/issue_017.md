...
- Decision: `valid`
- Notes:
  - The CLI create/update helpers currently trim `webhook_secret_value` before sending it to the daemon.
  - That mutated exact secret bytes before they even reached the API; I removed secret-value trimming in `internal/cli/automation.go` while leaving non-secret field normalization intact.
  - Verification: `make verify` passed with the CLI secret-preservation change.
