...
# Issue 008: _🛠️ Refactor suggestion_ | _🟠 Major_ | _⚡ Quick win_
## Review Comment
...
- Decision: `valid`
- Notes:
  - The MCP secret-ingestion test confirms server-side receipt of the write-only secret but does not assert that the HTTP response body stays redacted.
  - That left a real regression gap for a sensitive path; I added explicit non-leak assertions in `internal/api/core/settings_test.go` so the response body must not contain the raw secret.
  - Verification: `make verify` passed with the redaction assertion in place.
