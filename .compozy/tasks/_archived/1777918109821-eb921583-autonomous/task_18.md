...
## Overview
Execute the full QA pass for the autonomy MVP using the artifacts from task_17. This is the final quality gate: it must validate real backend, CLI/UDS, daemon lifecycle, coordination channel, web, and docs flows, fix root-cause regressions, and leave fresh evidence under `.compozy/tasks/autonomous/qa/`.
...
- MUST execute repository verification plus real backend/store, daemon, CLI/UDS, hook, coordination channel, scheduler, spawn, coordinator, web, and docs scenarios.
...
## Subtasks
- [x] 18.1 Activate `/qa-execution` with `qa-output-path=.compozy/tasks/autonomous` and derive execution matrix from task_17 artifacts.
- [x] 18.2 Run baseline repository verification gates and record pre-execution health.
- [x] 18.3 Execute real backend/store/daemon/CLI/UDS/hook/scheduler/spawn/coordinator scenarios from the QA matrix.
- [x] 18.4 Execute web Tasks UI and `packages/site` docs verification lanes.
- [x] 18.5 Fix root-cause regressions, add durable tests, and rerun narrow reproductions.
- [x] 18.6 Rerun final gates and publish `.compozy/tasks/autonomous/qa/verification-report.md`.
...
6. Coordinator and workers use the task-bound coordination channel for operational messages.
7. Coordinator delegates through safe spawn when allowed.
8. Scheduler recovers abandoned/expired work without claiming it.
9. Web and docs accurately describe manual-first autonomy.
...
- [ADR-012: Task-Run Coordination Channels](adrs/adr-012.md) - coordination channel validation.
...
- Unit/regression tests:
  - [x] Any bug found in config, hooks, task lease, scheduler, spawn, coordinator, or contract logic gains a focused regression.
  - [x] Any bug found in CLI/UDS payload handling gains handler or command tests.
  - [x] Any bug found in web Tasks UI gains adapter, hook, component, or Playwright regression coverage.
  - [x] Any bug found in docs/site gains link, build, typecheck, or content validation coverage where supported.
  - [x] Tests are strengthened to prove correct behavior; failing tests are not weakened to match broken production code.
- Integration/end-to-end tests:
  - [x] Real SQLite/store flows prove claim token redaction, capability matching, lease heartbeat, stale-token rejection, and expired-lease recovery.
  - [x] Real daemon/CLI/UDS flows prove `agh me`, `agh ch`, `agh task`, and `agh spawn` commands against isolated homes/workspaces.
  - [x] Real coordinator flow proves task creation does not start work, start/approval enqueues work with a stable coordination channel, and one workspace coordinator is bootstrapped.
  - [x] Real channel flow proves `status`/`blocker`/`result` messages carry task/run metadata, reject raw claim tokens, and do not mutate task-run status.
  - [x] Real scheduler flow proves wake/sweep/recovery without direct claiming.
  - [x] Real web flow proves task labels/actions match manual-first lifecycle.
  - [x] Real site/docs flow proves runtime autonomy and CLI docs are current.
  - [x] `make verify` plus required generated contract, web, and site gates pass after the final fix set.
...
- The autonomy MVP has fresh end-to-end evidence across runtime, CLI/UDS, hooks, coordination channels, scheduler, spawn, coordinator, web, and docs.
