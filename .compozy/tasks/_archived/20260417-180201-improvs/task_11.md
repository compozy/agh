...
- SCOPE IS THIS PACKAGE ONLY — only edit files inside `internal/sandbox/`. Cross-package findings go in the report under "Deferred Items". Editing outside the package = auto-fail.
...
- MUST write benchmarks in `internal/sandbox/`-co-located `*_bench_test.go` files for every hot-path candidate identified
...
- MUST land every "fixed" finding as code edits inside `internal/sandbox/`
...
- [x] 11.2 Map `internal/sandbox/`: list every Go file, public surface, goroutine entry points, and external callers (rg from repo root)
...
- [x] 11.4 Write benchmarks (`*_bench_test.go`) for every hot-path candidate; run `go test -bench=. -benchmem -count=5 ./internal/sandbox/...`; capture baseline numbers
...
- [x] 11.11 Apply fixes (Go code + tests + benchmark deltas) inside `internal/sandbox/`
...
- `internal/sandbox/` — sole edit scope (all Go files, including tests and new `*_bench_test.go`)
...
- All "fixed" findings applied as code changes in `internal/sandbox/`
...
- No edits outside `internal/sandbox/`
