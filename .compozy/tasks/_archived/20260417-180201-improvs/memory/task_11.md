...
## Objective Snapshot
- Execute the mandated improvements pass for `internal/sandbox/`: inventories first, then benchmarks, findings triage, in-package fixes only, final report, clean `make verify`, tracking updates, and one local commit.
...
- Treat the whole `internal/sandbox/` tree (`environment`, `daytona`, `local`, `providertest`, and the Daytona sidecar command) as in scope for inventories and in-package fixes.
...
## Files / Surfaces
- `internal/sandbox/types.go`, `registry.go`
- `internal/sandbox/local/provider.go`
- `internal/sandbox/providertest/suite.go`
- `internal/sandbox/daytona/{provider.go,tool_host.go,shell.go,sync.go,tar.go,ssh.go,sidecar_transport.go,sdk.go,launcher.go,state.go,env.go}`
