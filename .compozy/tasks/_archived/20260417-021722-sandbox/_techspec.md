...
3. Session manager allocates an `EnvironmentID`, persists `SessionEnvironmentMeta` in `creating` state, then calls `Provider.Prepare()` → Daytona provider creates or reattaches to a sandbox via SDK using AGH labels/tags (`agh_session_id`, `agh_environment_id`). Returns `Prepared` with runtime paths, provider state, and `Launcher`/`ToolHost`.
...
10. On session stop, session manager dispatches `environment.sync.before` through the canonical extension hook runtime, then calls `Provider.SyncFromRuntime(state, SyncReasonStop)` → streams the runtime workspace back as tar archives, applies the configured exclude policy, and writes results to the local workspace using last-write-wins semantics.
...
`EnvironmentID` is daemon-allocated before any provider call and is written to the session metadata in `creating` state. Remote providers must apply `agh_session_id` and `agh_environment_id` labels/tags during create. If the provider creates a remote sandbox but the API call times out before returning, restart reconciliation lists remote environments by those labels and either reattaches or destroys according to the persisted session state. `ProviderState` replaces the earlier generic `ResumeToken` placeholder and stores provider-specific reattach data such as Daytona sandbox metadata, archived state, and token issue metadata.
...
ALTER TABLE sessions ADD COLUMN environment_id TEXT DEFAULT '';
ALTER TABLE sessions ADD COLUMN environment_instance_id TEXT DEFAULT '';
ALTER TABLE sessions ADD COLUMN environment_state TEXT DEFAULT '';
ALTER TABLE sessions ADD COLUMN environment_provider_state_json TEXT DEFAULT '';
```
...
| `environment/info` | `{session_id: string}`                                 | `{environment_id, backend, profile, instance_id, runtime_root, sync_state, created_at, last_sync_error}` | Get details of a specific visible environment.                                              |
...
- **Lifecycle**: `Client.Create()`, `Sandbox.Start()`, `Stop()`, `Archive()`, `Delete()`. Create calls must attach AGH labels/tags (`agh_session_id`, `agh_environment_id`) before returning control to session lifecycle.
...
- **Partially failed provider create**: Simulate a Daytona create that succeeds remotely but times out locally. Restart daemon. Verify reconciliation finds the sandbox by `agh_environment_id`, reattaches when the session is recoverable, or destroys when unrecoverable.
- **Daemon restart environment cleanup**: Simulate daemon crash with active environment sessions. Restart daemon. Verify orphaned sandboxes are detected via provider labels and cleaned up.
...
- **Observability contract**: Verify logs/metrics/spans include `session_id`, `workspace_id`, `environment_id`, `backend`, `profile`, `instance_id`, durations, transfer sizes, and provider error kinds.
...
2. **Config environment profiles** (`internal/config/`) — depends on step 1 types. Add `EnvironmentProfile`, `DaytonaProfile`, `Defaults.Environment`, `Config.Environments`. Add validation and merge logic, including `DaytonaProfile.Snapshot` precedence over `Image` and explicit environment variable allowlist handling.
...
7. **Session environment integration** (`internal/session/`) — depends on steps 3 and 6. Inject environment manager/registry. Allocate `EnvironmentID`, persist `SessionEnvironmentMeta` in `creating` state before provider create, call `Prepare` in `startSession`, persist returned provider state, and call sync/destroy in lifecycle hooks. Add `environment_id`, `environment_backend`, `environment_instance_id`, `environment_state`, and `environment_provider_state_json` to sessions table.
...
10. **Daytona SSH transport** (`internal/environment/daytona/`) — depends on steps 1 and 9. Implement `sshTransport` using `golang.org/x/crypto/ssh`. Token management via Daytona REST API, with persisted expiry and proactive refresh at 50% of the token lifetime. Implement provider-internal `transport` interface.

11. **Daytona provider** (`internal/environment/daytona/`) — depends on steps 1, 10. Implement `Provider` (sandbox create/start/stop/archive/delete), `Launcher` (via SSH transport), `ToolHost` (via Daytona SDK filesystem + SSH terminals), snapshot-aware sandbox creation, AGH label/tag attachment, and session-bidirectional tar sync over SSH.

12. **Daemon restart environment cleanup** (`internal/daemon/boot.go`) — depends on step 8. Add environment reconciliation in `bootRuntime` after `cleanupOrphans` and after the canonical resource-runtime boot/reconcile phase when that runtime is enabled. Load persisted `SessionEnvironmentMeta` for all sessions. For sessions in non-terminal states with remote backends: attempt reattach via `Provider.Prepare()` using `EnvironmentID`, `InstanceID`, and `ProviderState`. If local metadata is missing `InstanceID` but remote labels contain the daemon-allocated `EnvironmentID`, reconcile the partial create by attaching the sandbox to the session or destroying it if the session is unrecoverable. Follows existing pattern of `observer.Reconcile()`.
...
- **Tracing**: Emit optional OpenTelemetry-compatible spans through the existing observability layer. Required span names: `environment.prepare`, `environment.sync.to_runtime`, `environment.sync.from_runtime`, `environment.transport.ssh.connect`, `environment.transport.ssh.refresh_token`, and `environment.destroy`. Required attributes: `session_id`, `workspace_id`, `environment_id`, `backend`, `profile`, `instance_id`, `duration_ms`, `files`, `bytes`, and `error_kind` when applicable.
...
- **Status/list surfaces**: Session list/get/status responses include `environment` field (`environment_id`, `backend`, `profile`, `state`, `instance_id`) for operator visibility. CLI `session list` shows backend/state columns. CLI `session info` shows full environment details including sync state and last sync error.
...
10. **Provider create is idempotent by daemon identity**. AGH allocates `EnvironmentID` before provider create, labels remote sandboxes with it, and persists provider-specific reattach state in `ProviderState` instead of relying on a vague resume token.
...
| Provider create succeeds remotely but times out locally | Medium     | Medium — orphan or duplicate sandbox | Persist `EnvironmentID` before create and tag remote sandboxes. Restart reconciliation attaches or destroys by `agh_environment_id` even when `InstanceID` was not written locally.             |
