...
## Objective Snapshot
- Implement Task 08: add five `environment.*` hook events/payloads/dispatchers/matchers, dispatch them from session environment prepare/sync/stop lifecycle points, add `environment/list`, `environment/info`, and `environment/exec` Host API methods with capability gating, and verify with tests plus `make verify`.

## Important Decisions
- `environment/exec` should run through the active session's environment `ToolHost` terminal interface, preserving provider abstraction for both local and Daytona environments.
...
- `environment/list` and `environment/info` require the manifest action grant only; `environment/exec` requires both the action grant and `environment.exec` security capability.

## Learnings
- Shared workflow memory says Task 04 already integrated session environment lifecycle and Task 07 completed boot-time environment reconciliation. Task 08 should build on those surfaces without adding a separate sandbox-specific extension registry.
...
- Baseline implementation gap confirmed: `internal/` currently has no environment hook events/dispatchers and no `environment/list`, `environment/info`, `environment/exec`, or `environment.exec` Host API mapping.
...
- Session environment sync now has to expose explicit options/results so `environment.sync.before` can pass `exclude_patterns` to providers and `environment.sync.after` can report files/bytes/error counts without parsing logs.
...
- Additional touched surfaces: `internal/environment` provider sync contracts/providers/tests and `internal/acp` because Host API `environment/exec` needs access to the prepared session `ToolHost`.
...
- `environment.stop` denial prevents provider destroy while still allowing session stop; tests assert the session reaches stopped state and the provider destroy count remains unchanged.
...
- Targeted verification passed:
  ...
  - `go test ./internal/acp ./internal/environment/... ./internal/hooks ./internal/session ./internal/extension/... ./internal/daemon`
...
- Created local source-only commit `457d4d64` (`feat: add environment extension hooks`); task tracking and memory files remain unstaged per workflow rules.
