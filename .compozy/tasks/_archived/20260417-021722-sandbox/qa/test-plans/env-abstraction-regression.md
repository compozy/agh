# Regression Suite: Execution Environment Abstraction

**Version:** 1.0
**Date:** 2026-04-16
**Feature:** Execution Environment Abstraction Layer
...
SMOKE-003: Config with environment profile loads without error
...
SMOKE-008: Daemon boot completes with environment registry wired
```
...
- Session environment lifecycle (TC-FUNC-008 to TC-FUNC-016)
...
| TC-FUNC-008 | Session start allocates EnvironmentID | Session |
...
| TC-FUNC-015 | Session resume restores environment metadata | Session |
...
| TC-INT-004 | Session list API includes environment field | Integration |
| TC-REG-005 | Existing ACP client_integration_test passes | ACP |
| TC-REG-006 | Existing session manager tests pass | Session |
| TC-FUNC-026 | environment.prepare deny aborts session creation | Hooks |

### P1 — Should Run Weekly+ (16 tests)

| ID | Title | Area |
|----|-------|------|
| TC-FUNC-001 | Valid environment profile parses from TOML | Config |
...
| TC-FUNC-005 | Environment overlay merge preserves fields | Config |
| TC-FUNC-006 | Defaults.Environment cascade resolves profile | Config |
| TC-FUNC-007 | Missing EnvironmentRef resolves to local | Config |
...
| TC-FUNC-023 | Reconciliation finds sandbox by agh_environment_id | Daemon |
...
| ID | Title | Area |
|----|-------|------|
| TC-FUNC-025 | EnvironmentProfile.Env map parses key-value pairs | Config |
| TC-FUNC-027 | environment.sync.before deny skips sync | Hooks |
| TC-FUNC-028 | environment.stop deny prevents destroy | Hooks |
| TC-FUNC-029 | Host API environment/list returns instances | Hooks |
| TC-FUNC-030 | Host API environment/exec requires capability | Hooks |
...
| TC-INT-007 | Workspace CRUD exposes environment_ref via API | Integration |
| TC-INT-008 | CLI workspace add --environment flag | Integration |
| TC-PERF-002 | Session start latency with local provider | Performance |
| TC-PERF-003 | Environment reconciliation boot time | Performance |
