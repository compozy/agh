...
- Decision: `valid`
- Root cause: `detectCapabilityCatalogLayout()` relies on `existingCapabilityCatalogFile()` and `existingCapabilityCatalogDir()` to distinguish absent paths from present reserved catalog locations. Both helpers currently return `(false, nil)` when the path exists with the wrong type, so `LoadAgentCapabilities()` silently treats a misconfigured reserved path as if no catalog were present.
- Evidence: in `internal/config/capabilities.go`, `existingCapabilityCatalogFile()` returns `!info.IsDir()` and `existingCapabilityCatalogDir()` returns `info.IsDir()`, which collapses "exists with wrong type" into the same result as `os.ErrNotExist`.
- Fix approach: make both helpers fail fast with descriptive errors for wrong-type reserved paths and add `LoadAgentCapabilities()` regression tests covering file-vs-directory mismatches on the reserved catalog paths.
- Resolution: `existingCapabilityCatalogFile()` now errors when a reserved catalog file path is a directory, and `existingCapabilityCatalogDir()` now errors when the reserved directory path is a regular file, so `LoadAgentCapabilities()` surfaces the misconfiguration instead of silently skipping the catalog.
- Verification:
