...
- Decision: `valid`
- Notes:
  - Verified against the current test body in `internal/api/core/network_test.go`: both scoped subtests only assert `resp.Code`, so they would still pass if a different 400/404 branch produced the same status.
  - The handler already returns structured `contract.ErrorPayload` responses via `RespondError`, and nearby tests in the same file already decode that payload and assert message substrings.
  - Root cause: the new error-path coverage stops at transport status and does not validate the actual failure reason or run the independent subtests in parallel.
  - Fix approach: add `t.Parallel()` to both subtests, decode `contract.ErrorPayload`, and assert the real error messages for the not-found and invalid-limit branches.
  - Resolved in `internal/api/core/network_test.go` by adding `t.Parallel()` to both subtests and asserting `contract.ErrorPayload.Error` contains the expected not-found and invalid-integer messages.
  - Verification:
