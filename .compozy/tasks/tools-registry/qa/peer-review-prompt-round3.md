...
- ADRs:
  ...
  - /Users/pedronauck/Dev/compozy/agh/.compozy/tasks/tools-registry/adrs/adr-011-official-go-mcp-sdk.md
- Research:
  ...
  - /Users/pedronauck/Dev/compozy/agh/.compozy/tasks/tools-registry/analysis/analysis_go_sdk_mcp.md
...
REVIEW FOCUS:
- This is a delta review focused on the newly adopted official MCP Go SDK boundary.
- Pressure-test whether the spec cleanly distinguishes protocol/transport reuse from AGH-owned policy/auth/manageability boundaries.
- Pressure-test whether the SDK adoption creates hidden duplication with `internal/mcp/auth` or weakens hosted MCP bind/approval invariants.
- Pressure-test whether the TechSpec still gives implementers concrete direction for stdio-hosted MCP, stdio external MCP, and remote streamable HTTP MCP.

YOUR JOB:
1. Read every context file fully before reasoning.
2. Identify BLOCKERS (issues that prevent approval): unsound boundary definitions, hidden auth duplication, under-specified hosted MCP behavior after SDK adoption, security regressions, partial-surface completion, or implementation ambiguity caused by the SDK delta.
3. Identify NITS (non-blocking improvements): clarity, naming, test density, better delete targets, missing evidence references.
