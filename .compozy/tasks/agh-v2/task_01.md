...
## Subtasks
- [ ] 1.1 Define all config structs (Config, DaemonConfig, HTTPConfig, LimitsConfig, PermissionsConfig, ProviderConfig, ObservabilityConfig, LogConfig)
- [ ] 1.2 Implement TOML loading with 2-level merge (global + workspace deep-merge)
- [ ] 1.3 Implement config validation (required fields, value ranges, port bounds)
- [ ] 1.4 Implement HomePaths struct and EnsureHomeLayout for directory creation
- [ ] 1.5 Implement AgentDef parsing from AGENT.md (YAML frontmatter + Markdown body extraction)
- [ ] 1.6 Implement built-in provider registry with resolution chain
- [ ] 1.7 Implement MCPServer merging logic (agent-level + provider-level)
- [ ] 1.8 Implement `internal/logger` package (slog setup with configurable level + file output to `~/.agh/logs/agh.log`)
- [ ] 1.9 Implement `internal/version` package (build metadata injection via ldflags)
- [ ] 1.10 Implement .env loading via godotenv in config loading path
...
## Tests
- Unit tests:
  - [ ] Load valid TOML config with all sections
  - [ ] 2-level merge: workspace overrides global values correctly
  - [ ] 2-level merge: workspace adds new values without clobbering global
  - [ ] Validation: reject invalid port (0, 65536+)
  - [ ] Validation: reject unknown permission mode
  - [ ] AGH_HOME env var overrides default home location
  - [ ] EnsureHomeLayout creates all required directories
  - [ ] Parse AGENT.md: valid frontmatter + Markdown body
  - [ ] Parse AGENT.md: missing required fields (name, provider) returns error
  - [ ] Built-in provider registry returns correct commands for known agents
  - [ ] Provider config.toml override merges with built-in defaults
  - [ ] Resolution chain: AGENT.md model overrides provider default_model
  - [ ] MCP servers merge: agent-level + provider-level combined
  - [ ] Load config from non-existent file returns defaults (not error)
