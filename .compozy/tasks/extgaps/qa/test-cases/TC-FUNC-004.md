...
5. Verify job IDs are stable hashes
   **Expected:** job IDs start with "job_" and are deterministic

6. Verify trigger IDs are stable hashes
   **Expected:** trigger IDs start with "trg_"

7. Verify bridge instance was inserted into store
   **Expected:** Bridge with ID starting "bri_", Source=BridgeInstanceSourcePackage, Enabled=false, Status=BridgeStatusDisabled
