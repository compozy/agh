...
## Overview
Expose task-run claiming and lease maintenance to agents through first-class UDS endpoints and CLI verbs. Agents should claim, heartbeat, complete, fail, and release work without operator-only commands or private daemon calls.

<critical>
- ALWAYS READ `_techspec.md`, ADR-002, ADR-003, ADR-010, and ADR-011 before adding agent task verbs
...
## Subtasks
- [ ] 9.1 Add agent task UDS routes and handlers for claim-next and lease mutations.
- [ ] 9.2 Add CLI client methods and `agh task next|heartbeat|complete|fail|release` verbs with JSON output.
- [ ] 9.3 Wire caller identity, permissions, capability criteria, and structured audit actor fields.
- [ ] 9.4 Enforce claim-token redaction in command errors, logs, and non-claim responses.
- [ ] 9.5 Add handler/CLI tests for success, stale token, denied caller, malformed payload, and no-work-found cases.
- [ ] 9.6 Run generated contract/web checks if HTTP/OpenAPI-facing DTOs change.
...
Claim-next should return enough context for an agent to execute the task: run ID, task identity, instructions/metadata needed by the current task model, lease deadline, raw claim token, and redacted audit metadata.
...
## Tests
- Unit tests:
  - [ ] Handler rejects unauthenticated callers and callers without task lease permission.
  - [ ] CLI payload parsing validates token/run IDs and produces stable JSON errors.
  - [ ] Claim response includes raw token once; heartbeat/complete/fail/release responses do not echo it.
  - [ ] No-work-found returns a structured non-error or documented exit code suitable for agent loops.
  - [ ] Existing operator task commands continue to parse and call the operator service paths.
- Integration tests:
  - [ ] A session claims a queued run, heartbeats it, completes it, and cannot complete it again with the old token.
  - [ ] A stale token cannot release or fail a run after lease recovery.
  - [ ] CLI commands work over UDS using agent identity environment from task_05.
  - [ ] Manual user-started and agent-created task runs both flow through the same API.
  - [ ] Generated OpenAPI/web typecheck/web tests pass if public DTOs change.
