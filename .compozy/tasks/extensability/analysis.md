...
| Health endpoint | **Exists** | `GET /api/observe/health` returns uptime, active sessions, DB sizes, version |
...
| **No session stop reason** | 4 binary states. No classification of *why* a session stopped (user cancel, crash, timeout, budget, loop). | `SessionInfo` has no `StopReason` field. `finalizeStopped()` records an error event but doesn't classify. |
...
- Support 2 hook executor types initially (3rd added by extension architecture later):
  ...
  - *(Future, via extension architecture)* **Wasm hooks** for sandboxed in-process execution
