...
- [ ] 28.1 Read `_techspec.md` end-to-end (especially Anti-Evasion Hard Rules, Mandatory Per-Skill Artifacts, Failure Modes); confirm scope
- [ ] 28.2 Map `internal/store/`: list every Go file, public surface, goroutine entry points, and external callers (rg from repo root)
- [ ] 28.3 Build the five mandatory inventories (cyclomatic top-10, hot-path candidates, goroutine/channel/mutex/select tables, attacker-input surfaces) and write them to the report BEFORE running fixes
- [ ] 28.4 Write benchmarks (`*_bench_test.go`) for every hot-path candidate; run `go test -bench=. -benchmem -count=5 ./internal/store/...`; capture baseline numbers
- [ ] 28.5 Run $refactoring-analysis against the file-size + duplication + cyclomatic inventory; record findings
- [ ] 28.6 Run $extreme-software-optimization against the benchmarked candidates; record findings (only fixes with measurable improvement count as "fixed")
- [ ] 28.7 Invoke $ubs via the Skill tool; capture output excerpt OR mark not-run with literal refusal message
- [ ] 28.8 Run $deadlock-finder-and-fixer against the goroutine/channel/mutex/select inventories; record findings
- [ ] 28.9 Run $security-review after writing the threat model and the attacker-input surface inventory; per-surface verdict required
- [ ] 28.10 Triage all findings into fixed / deferred / wontfix with reasons
- [ ] 28.11 Apply fixes (Go code + tests + benchmark deltas) inside `internal/store/`
- [ ] 28.12 Run `make verify`; fix root causes until clean; capture final excerpt
- [ ] 28.13 Re-run benchmarks; populate before/after numbers in the optimization table
- [ ] 28.14 Verify the report against `_techspec.md` "Failure Modes" — every `run` skill has its artifact section; every "no findings" carries an inventory
- [ ] 28.15 Run $cy-final-verify before flipping status
