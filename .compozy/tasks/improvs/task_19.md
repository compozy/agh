...
- [ ] 19.1 Read `_techspec.md` end-to-end (especially Anti-Evasion Hard Rules, Mandatory Per-Skill Artifacts, Failure Modes); confirm scope
- [ ] 19.2 Map `internal/memory/`: list every Go file, public surface, goroutine entry points, and external callers (rg from repo root)
- [ ] 19.3 Build the five mandatory inventories (cyclomatic top-10, hot-path candidates, goroutine/channel/mutex/select tables, attacker-input surfaces) and write them to the report BEFORE running fixes
- [ ] 19.4 Write benchmarks (`*_bench_test.go`) for every hot-path candidate; run `go test -bench=. -benchmem -count=5 ./internal/memory/...`; capture baseline numbers
- [ ] 19.5 Run $refactoring-analysis against the file-size + duplication + cyclomatic inventory; record findings
- [ ] 19.6 Run $extreme-software-optimization against the benchmarked candidates; record findings (only fixes with measurable improvement count as "fixed")
- [ ] 19.7 Invoke $ubs via the Skill tool; capture output excerpt OR mark not-run with literal refusal message
- [ ] 19.8 Run $deadlock-finder-and-fixer against the goroutine/channel/mutex/select inventories; record findings
- [ ] 19.9 Run $security-review after writing the threat model and the attacker-input surface inventory; per-surface verdict required
- [ ] 19.10 Triage all findings into fixed / deferred / wontfix with reasons
- [ ] 19.11 Apply fixes (Go code + tests + benchmark deltas) inside `internal/memory/`
- [ ] 19.12 Run `make verify`; fix root causes until clean; capture final excerpt
- [ ] 19.13 Re-run benchmarks; populate before/after numbers in the optimization table
- [ ] 19.14 Verify the report against `_techspec.md` "Failure Modes" — every `run` skill has its artifact section; every "no findings" carries an inventory
- [ ] 19.15 Run $cy-final-verify before flipping status
