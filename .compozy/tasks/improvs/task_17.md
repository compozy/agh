...
- [ ] 17.1 Read `_techspec.md` end-to-end (especially Anti-Evasion Hard Rules, Mandatory Per-Skill Artifacts, Failure Modes); confirm scope
- [ ] 17.2 Map `internal/hooks/`: list every Go file, public surface, goroutine entry points, and external callers (rg from repo root)
- [ ] 17.3 Build the five mandatory inventories (cyclomatic top-10, hot-path candidates, goroutine/channel/mutex/select tables, attacker-input surfaces) and write them to the report BEFORE running fixes
- [ ] 17.4 Write benchmarks (`*_bench_test.go`) for every hot-path candidate; run `go test -bench=. -benchmem -count=5 ./internal/hooks/...`; capture baseline numbers
- [ ] 17.5 Run $refactoring-analysis against the file-size + duplication + cyclomatic inventory; record findings
- [ ] 17.6 Run $extreme-software-optimization against the benchmarked candidates; record findings (only fixes with measurable improvement count as "fixed")
- [ ] 17.7 Invoke $ubs via the Skill tool; capture output excerpt OR mark not-run with literal refusal message
- [ ] 17.8 Run $deadlock-finder-and-fixer against the goroutine/channel/mutex/select inventories; record findings
- [ ] 17.9 Run $security-review after writing the threat model and the attacker-input surface inventory; per-surface verdict required
- [ ] 17.10 Triage all findings into fixed / deferred / wontfix with reasons
- [ ] 17.11 Apply fixes (Go code + tests + benchmark deltas) inside `internal/hooks/`
- [ ] 17.12 Run `make verify`; fix root causes until clean; capture final excerpt
- [ ] 17.13 Re-run benchmarks; populate before/after numbers in the optimization table
- [ ] 17.14 Verify the report against `_techspec.md` "Failure Modes" — every `run` skill has its artifact section; every "no findings" carries an inventory
- [ ] 17.15 Run $cy-final-verify before flipping status
