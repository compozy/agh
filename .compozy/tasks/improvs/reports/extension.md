...
| `internal/extension/host_api.go:913-933` | Extension-supplied environment exec request (`session_id`, `command`, `timeout`) | Non-empty validation plus upstream `CheckHostAPI(..., "environment/exec")` capability gate in `Handle`. | `sessions.ExecEnvironment(ctx, session.EnvironmentExecRequest{...})`. | LOW — rejected because execution requires explicit capability grant and active-session authorization in downstream session handling. |
