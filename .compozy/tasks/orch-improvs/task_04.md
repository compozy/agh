---
status: completed
title: "Review-Gate GlobalDB Schema Foundation"
type: backend
complexity: high
dependencies:
  - task_02
...
# Task 04: Review-Gate GlobalDB Schema Foundation

## Overview
This task introduced the persistent review-gate schema. It added review policy fields, `task_run_reviews`, and review trigger/continuation columns without modeling reviews as an intermediate run status.
...
<requirements>
- MUST add review-gate schema through a numbered migration.
- MUST represent reviews post-terminal through review rows and continuation links.
- MUST keep fresh and migrated database schemas aligned.
</requirements>

## Subtasks
- [x] Add migration v18 for review policy and run-review tables.
- [x] Add fresh schema statements for review gate columns and indexes.
- [x] Preserve task-table rebuild behavior for existing data.
- [x] Cover fresh and migrated review schema with tests.
...
### Relevant Files
- `internal/store/globaldb/migrate_task_review_gate.go` - review-gate migration.
- `internal/store/globaldb/schema_task_review_gate.go` - review-gate DDL.
- `internal/store/globaldb/schema.go` - fresh schema integration.

### Dependent Files
- `internal/store/globaldb/schema_test.go` - review schema assertions.
- `internal/store/globaldb/migrations_test.go` - migrated schema coverage.
...
- Extensibility: Established persistent review authority for reviewer skills/tools.
- Agent manageability: No direct agent surface yet; later native and transport tasks expose the authority.
- Config lifecycle: Used typed review policy from task 01.

### Web/Docs Impact
- `web/`: No direct `web/` change.
- `packages/site`: Review-gate narrative docs deferred to task 29.
...
## Completion Evidence
- State: `state.yaml.progress.checklist` iteration 9 is `completed`.
- Memory: `memory/free-iter-008.md`.
