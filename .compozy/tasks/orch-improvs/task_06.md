---
status: completed
title: "Durable Notification Cursor Primitive"
type: backend
complexity: high
dependencies:
  - task_02
...
# Task 06: Durable Notification Cursor Primitive

## Overview
This task introduced durable notification cursors as a reusable primitive. It added typed cursor APIs, monotonic advance semantics, replay idempotency, reset handling, and GlobalDB persistence.
...
<requirements>
- MUST persist notification cursor identity and sequence state durably.
- MUST advance cursors monotonically and reject stale advances.
- MUST expose typed service errors instead of stringly error checks.
</requirements>

## Subtasks
- [x] Create the `internal/notifications` cursor domain and service contract.
- [x] Add GlobalDB migration and fresh schema for `notification_cursors`.
- [x] Implement cursor CRUD, monotonic advance, replay, and reset semantics.
- [x] Add unit and store tests for cursor behavior.
...
### Relevant Files
- `internal/notifications` - cursor domain and service.
- `internal/store/globaldb/global_db_notification_cursor.go` - cursor store.
- `internal/store/globaldb/migrate_notification_cursors.go` - cursor migration.

### Dependent Files
- `internal/store/globaldb/global_db_notification_cursor_test.go` - store behavior.
- `internal/notifications` tests - service validation.

### Related ADRs
- [ADR-003: Durable Cursor Primitive](adrs/adr-003.md) - notification cursors are monotonic and replay-safe.

## Extensibility / Agent Manageability / Config Lifecycle
- Extensibility: Cursor service becomes the primitive for bridge notifications and SSE seeding.
- Agent manageability: Later CLI/HTTP/UDS diagnostics expose cursor state in task 25.
- Config lifecycle: No config changes.

### Web/Docs Impact
- `web/`: Cursor state feeds notification diagnostics in tasks 26 and 27.
- `packages/site`: Cursor semantics documented in task 29.
...
## Completion Evidence
- State: `state.yaml.progress.checklist` iteration 13 is `completed`.
- Memory: `memory/free-iter-012.md`.
