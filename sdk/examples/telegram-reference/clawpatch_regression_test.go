...
func TestTelegramReferenceShutdownCancellation(t *testing.T) {
	...
	t.Run("Should cancel blocked ingest host call before shutdown drain expires", func(t *testing.T) {
		...
		managed := telegramRegressionManagedInstance(now, "brg-telegram-reference")
		handleTelegramRegressionLifecycle(t, hostPeer, managed)
		...
		mustHandle(
			...
			func(ctx context.Context, params json.RawMessage) (any, error) {
				...
				case <-ctx.Done():
					...
					return nil, ctx.Err()
				case <-release:
					return telegramRegressionIngestResult(envelope), nil
				}
			},
		)

		initializeTelegramRegressionRuntime(t, hostPeer, now, managed)
		if err := appendJSONLine(
			env.updatesPath,
			telegramRegressionUpdate(now, managed.Instance.ID, 9001, "blocked ingest"),
		); err != nil {
...
func TestTelegramReferenceAuthStatus(t *testing.T) {
	t.Run("Should classify whitespace-only cached bot token as auth required", func(t *testing.T) {
		...
		managed := telegramRegressionManagedInstance(now, "brg-telegram-reference")
		handleTelegramRegressionLifecycle(t, hostPeer, managed)
		recordTelegramRegressionIngests(t, hostPeer)
		initializeTelegramRegressionRuntime(t, hostPeer, now, managed)
...
func TestTelegramReferenceMalformedUpdateProgress(t *testing.T) {
	...
	t.Run("Should skip malformed update line and ingest later valid update", func(t *testing.T) {
		...
		managed := telegramRegressionManagedInstance(now, "brg-telegram-reference")
		handleTelegramRegressionLifecycle(t, hostPeer, managed)
		ingested := recordTelegramRegressionIngests(t, hostPeer)

		validLine, err := json.Marshal(telegramRegressionUpdate(
			now,
			managed.Instance.ID,
			9002,
		...
		initializeTelegramRegressionRuntime(t, hostPeer, now, managed)
		waitForTelegramRegressionCondition(t, func() bool {
			items := ingested()
			return len(items) > 0 &&
...
func TestTelegramReferenceSideEffectAppend(t *testing.T) {
	t.Run("Should preserve pre-existing file contents on first append after restart", func(t *testing.T) {
		resetTelegramRegressionSideEffectSnapshots()
		t.Cleanup(resetTelegramRegressionSideEffectSnapshots)
...
func telegramRegressionManagedInstance(
	now time.Time,
	instanceID string,
...
func handleTelegramRegressionLifecycle(
	t *testing.T,
...
func recordTelegramRegressionIngests(
	t *testing.T,
	hostPeer *bridgesdk.Peer,
...
func telegramRegressionIngestResult(
...
func initializeTelegramRegressionRuntime(
	t *testing.T,
	hostPeer *bridgesdk.Peer,
	now time.Time,
	managed subprocess.InitializeBridgeManagedInstance,
...
func waitForTelegramRegressionCondition(t *testing.T, condition func() bool) {
	t.Helper()
...
func resetTelegramRegressionSideEffectSnapshots() {
