...
- **Eyebrow markup is mandatory.** Every uppercase JetBrains Mono label in marketing/docs MUST render through `<Eyebrow>` from `@agh/ui` (`case="upper"`, `size="eyebrow|badge|micro"`, `tone`). Inlining `font-mono` + `uppercase` + `text-[Npx]`/`tracking-[Nem]` tuples is forbidden — the canonical tokens are `--text-eyebrow` / `--text-badge` / `--text-micro` + `--tracking-mono` (0.06em). See `DESIGN.md` §3 and `web/CLAUDE.md` for the full rule.
