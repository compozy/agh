...
`agh provider auth login` requires the provider to define `auth_login_command`; otherwise run the
provider's own login command, such as `claude auth login`, `codex login`, or `opencode auth login`.
