...
command or run the provider's own login command outside AGH. AGH prints the resolved command for
an interactive terminal and does not execute native login flows itself. For isolated provider homes,
the printed command includes the required environment prefix so native credentials land in AGH's
provider home. Use `agh provider auth login <provider> --print-command` for one raw copyable shell
command.
