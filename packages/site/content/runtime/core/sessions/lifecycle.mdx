...
`name` is optional for user sessions. When it is omitted, AGH derives a durable title from the
first submitted prompt, persists that title in session metadata and the catalog, and keeps it across
stop and resume. An explicit name always wins, and daemon-managed session types remain unnamed
unless their creator supplies an identity.
