...
description: Metadata-only session presence, attachability, wake eligibility, and the boundary between session activity supervision, task lease heartbeat, authored HEARTBEAT.md policy, and AGH Network greet presence.
---
...
<OperatorNote>
  ...
  session health|status|inspect` commands, and `GET /api/sessions/:id/health|status|inspect`.
</OperatorNote>
...
- Session health is **not** AGH Network presence. Peer presence and `greet_interval` belong to
  the [protocol model](/runtime/core/network/protocol). A peer being online never makes a session
