...
| ------------------------ | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| Persisted session stream | `GET /api/workspaces/:workspace_id/sessions/:session_id/stream`  | Stored `SessionEventPayload` rows with stable sequence numbers and SSE replay support       |
...
```bash
curl -N http://localhost:2123/api/workspaces/ws_alpha/sessions/sess-1234/stream
```
...
- `GET /api/workspaces/:workspace_id/sessions/:session_id/transcript` assembles canonical replay messages from these rows
