...
Config-backed webhook triggers need an endpoint slug and an environment variable containing the
secret. AGH resolves the secret when it loads and validates config.
...
This GitHub Actions step notifies AGH after a deployment workflow reaches the point where an agent
