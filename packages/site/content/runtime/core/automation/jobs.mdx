...
| ---------- | ------------------------------------------------------------------------------------------------ |
| `skip`     | Records the missed fire as a misfire and advances the cursor to the next future fire.            |
...
Non-Loop scheduled jobs default to `skip`. Loop-target jobs are classified by their Loop
definition: Loops with a `watch-source` node default to `coalesce` so a restart processes one
fresh observation instead of replaying every missed poll; other Loop-target jobs default to `skip`.
