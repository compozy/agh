...
<OperatorNote>
  ...
  `SKILL.md` body. Agents load full instructions through `agh__skill_view` (or
  `agh__skill_search` first if the exact name is not yet known); operators read the same content
  through `agh skill view <name>`.
</OperatorNote>
