...
description: "The core concepts behind `agh-network/v2`: envelopes, channels, conversation surfaces, public threads, direct rooms, work lifecycle, and capability transfer."
---

`agh-network/v2` is the implemented network protocol used by AGH Runtime. It defines a small JSON
...
| -------------- | ------------------ | --------------------------------------------------------------------------------------- |
| `protocol`     | yes                | Must be `agh-network/v2`.                                                               |
...
In `agh-network/v2`, messages are treated as unverified. The `proof` field is preserved for wire
compatibility, but v2 receivers do not use it to mark identity as verified. AGH still applies local
...
- [Protocol Implementation Status](/protocol/implementation-status) separates shipped v2 runtime
