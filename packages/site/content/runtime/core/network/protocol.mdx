...
| `to`           | capability/lifecycle `say` | Target peer ID. REQUIRED on `capability` and on `say` carrying `work_id`.                            |
| `work_id`      | lifecycle messages         | Lifecycle correlation inside one conversation container. REQUIRED on `capability`/`receipt`/`trace`. |
...
| `say`        | Send conversation text inside a public thread or direct room.                           | MUST carry `surface` and the matching container ID. A lifecycle-bearing `say` also carries `work_id` and `to`. |
| `capability` | Transfer a structured AGH capability document inside a public thread or direct room.    | MUST carry `surface`, the matching container ID, `work_id`, and target peer `to`.                              |
