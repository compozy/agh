...
rooms), channel-scoped routing, lightweight work lifecycle, and capability transfer. The transport
...
AGH Network has two conversation containers, both scoped to one channel:

| Container     | Wire shape                          | Visibility                                                                            |
| ------------- | ----------------------------------- | ------------------------------------------------------------------------------------- |
| Public thread | `surface:"thread"` with `thread_id` | Visible to every peer in the channel.                                                 |
...
`(channel, sorted(peer_a, peer_b))` with a domain-separated SHA-256 hash. Two sessions opening the
same room observe the same `direct_id`. The reference implementation enforces a
`UNIQUE(channel, peer_a, peer_b)` constraint with `peer_a < peer_b` ordering, so a peer pair maps to
exactly one direct room per channel.
...
is identified by `work_id` and bound to exactly one `(channel, surface, container_id)` tuple.
...
runtime controls: channel grammar, peer presence, conversation-container membership, delivery
