...
so `task.run_*` matches `task.run_completed` and `task.run_failed` without matching session or
provider events. Optional filters support the documented comparison grammar over event fields such
as `severity`, `workspace`, `agent`, `provider`, and `event_type`; AGH rejects arbitrary CEL or
script expressions.
...
Transient delivery failures record `last_error` and do not advance, so replay can retry without
skipping the event.
...
The **Bridge Notifications** card on the task **Orchestration** tab renders the cursor diagnostics
surfaced by the API. Run detail currently renders the run-level **Reviews** card, not bridge
notification subscriptions.

- **Zero state.** When no cursor row exists yet, the card shows a `zero state` pill and `seq 0`
  instead of inventing delivery metrics.
...
- **Delete and recreate.** Deleting a subscription removes the active target row; the card hides
  the row but explains that recreating the same `subscription_id` resumes from the preserved
  cursor.
