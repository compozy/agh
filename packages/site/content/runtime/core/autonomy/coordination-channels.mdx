...
| `workflow_id`             | no       | Optional correlation key. It is metadata in the MVP, not a first-class runtime store. |
...
Raw lease credential fields are rejected in coordination metadata and must not appear in channel
