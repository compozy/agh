...
6. The daemon binds the reviewer session to the review request through
   `task.Service.BindRunReviewSession` before exposing `submit_run_review`. The bundled
   `agh-task-reviewer` skill loads only because the binding is durable.
...
- The bundled `agh-task-reviewer` skill must be active. Its `metadata.agh.requires_review_request`
  flag is the load trigger; the tool is hidden until the loader sees an active review binding.
- `LookupReviewForSession(session_id)` must return a binding that matches the call's `review_id`.
  Mismatched ids return an unavailable-tool error and never reach the verdict path.
...
Open a task and switch to the **Orchestration** tab (the same tab documented in
[Task Execution Profiles](/runtime/core/autonomy/execution-profiles)). The task-level reviews card
and the run-detail page's run-level variant are read-only views over the same authority:
...
- A permanent disclaimer reinforces that operator sessions cannot submit a verdict from the web UI.
  Verdict authority is the reviewer-bound native tool or the explicit
  `/api/task-reviews/{id}/verdict` endpoint.
...
See [Bundled Skills — Orchestration](/runtime/core/skills/bundled#agh-task-reviewer) for skill load
triggers and the orchestration-skill authority boundary.
...
- [Bundled Skills](/runtime/core/skills/bundled) documents the orchestration skills, including
  `agh-task-reviewer`.
