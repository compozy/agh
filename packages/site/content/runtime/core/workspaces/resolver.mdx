...
<OperatorNote>
  ...
  workspace remove` only removes the registration. Your project files stay on disk. AGH creates
  `<workspace>/.agh/workspace.toml` on first daemon touch when `[memory.workspace] auto_create =
  true`; commit it to git so collaborators share the same workspace identity.
</OperatorNote>
