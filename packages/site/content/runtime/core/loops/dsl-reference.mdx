...
| --------- | -------- | ------------------------------------------------------ |
| `action`  | **open** | `run-agent`, `run-loop`, `transform`, or any tool ID   |
| `control` | closed   | `fan-out`, `collect`, `branch`, `gate`, `sub-loop`     |
| `source`  | closed   | `input`, `file-import`, `watch-source`, `watch-events` |

### Action nodes

Three action kinds are reserved; every other action `kind` is a literal **tool ID**
...
Phase C (`coordinator`, `event`) extends this matrix as it lands; the kind select and the lint error
text always name the currently supported set, so the grammar never changes as families are added.
