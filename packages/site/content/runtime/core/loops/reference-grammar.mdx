...
— no arbitrary helpers. CEL conditions are compiled and cost-limited at publish time.
...
graph:
  nodes:
    - { id: load_tasks, class: source, kind: file-import, pattern: "tasks/*.md", parse: md_tasks }
    - {
        id: run_task,
        class: action,
        kind: run-agent,
