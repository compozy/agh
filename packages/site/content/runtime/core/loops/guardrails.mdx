...
| `no_op`     | The run executed but found nothing to do. A clean watch tick is `no_op`, never a fake `done`.                                                                                                          |
...
<OperatorNote label="Never coerced">
  `no_op` and `blocked` are never coerced to `done` or `failed`; exhaustion and stalling are always
