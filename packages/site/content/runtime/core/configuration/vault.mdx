...
`agh mcp install` supports two secret modes. A typed value creates a scope-qualified canonical ref;
a `--vault-ref KEY=vault:mcp/...` assignment binds an existing, present ref anywhere inside the
`mcp` namespace.
...
shared ref supplied through `--vault-ref`. Settings reads expose the bound ref strings but never
