...
| ------------------------------------ | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| `[daemon]`                           | UDS socket, process-memory reports, and task-run escalation from subprocess health.  | `socket = "$AGH_HOME/daemon.sock"`, memory reports every `5m`, escalation after `3` failed checks |
...
| `[session]`                          | Daemon-owned automatic title generation for unnamed user sessions.                   | `auto_title_enabled = true`                                                                       |
...
| `[marketplace.catalog]`              | Curated marketplace feed location and fetch timing.                                  | AGH `main` catalog, 1 h TTL, 10 s timeout                                                         |
...
| `[extensions.marketplace]`           | Extension registry override and side-load policy.                                    | registry unset, unverified side-loads disabled                                                    |
...
| `[loops.defaults.delivery]`          | Global/workspace seed defaults for delivery loops.                                   | iteration cap 50, no-progress window 3, gate revisions 10, fan-out 4                              |
| `[loops.defaults.watch]`             | Global/workspace seed defaults for watch loops.                                      | no iteration cap, no-progress window 2, fan-out 2                                                 |
...
[session]
# Generate one durable title after the first assistant response is persisted.
...
# Empty strings keep the provider/runtime default model.
worker = ""
judge = ""
...
# Watch loops run until stopped unless a loop or run override sets a cap.
iteration_cap = 0
fan_out_width = 2
...
| `subprocess_health_escalation_threshold` | integer     | `3`                     | `0` or a positive integer.                 | Failed health checks before AGH moves a task-bound nonterminal run to `needs_attention`; `0` disables the transition. |
...
`agh doctor -o json` reports Go heap allocation, heap in-use bytes, goroutines, uptime, and resident
...
subprocess. At the configured consecutive-failure count, AGH moves the exact task run bound to that
...
Set the field to `0` when you want `agh status` and `agh doctor` evidence without task mutation.
...
Settings › Layouts, while agents can use `agh config` or the typed
...
`top_center` and `bottom_center` accept `zoom`, `reserved`, or `none`. A target-less `stack` action
is invalid because stacking requires an occupied target window. `[window_manager.shortcuts]` maps
window command-registry IDs to unique, non-empty chord strings; conflicting chords reject the whole
candidate. Named any/landscape/portrait layout profiles are declarative `window_layout` resources,
...
| Field                | Type    | Default | Valid values      | Description                                                                                                |
| -------------------- | ------- | ------- | ----------------- | ---------------------------------------------------------------------------------------------------------- |
| `auto_title_enabled` | boolean | `true`  | `true` or `false` | Starts one bounded title pass for an unnamed user session after its first assistant response is persisted. |

An explicit session name always wins. If generation is disabled or fails, the session remains
unnamed; AGH does not derive a title from prompt text in the client or during prompt admission.
The setting is global or workspace desired state, is agent-mutable through `agh config set` and
the `agh__config_*` native tools, and follows the restart-required `session.*` lifecycle.
...
usage update reports context pressure. AGH first updates the workspace checkpoint with durable
...
These paths are global or workspace desired state and are available through `agh config set` and
the `agh__config_*` native tools. Structured writes return `lifecycle="restart-required"` and
`next_action="restart-daemon"`; the running daemon keeps its bound compaction policy until restart.
Use `agh config apply-history -o json` to inspect the blocked apply record.
...
[Agent Soul](/runtime/core/agents/soul).
...
[Agent Heartbeat](/runtime/core/agents/heartbeat).
...
When a post-hook, redacted tool result exceeds its effective result cap, AGH retains the canonical
...
| Field     | Type     | Default | Valid values       | Description                                                            |
| --------- | -------- | ------- | ------------------ | ---------------------------------------------------------------------- |
| `timeout` | duration | `5m`    | `1s` through `24h` | Maximum wait before AGH returns `{choice:null,text:"",fallback:true}`. |
...
OAuth-enabled remote servers store only metadata in config. Token material is encrypted in the AGH
Vault and is exposed through API, settings, CLI, logs, and the agent-callable `agh__mcp_status` /
`agh__mcp_auth_status` tools only as redacted status. Authorization and logout are daemon-mediated
...
| `client_secret_ref` | string       | no          | `env:NAME` or scope-qualified `vault:mcp/**/oauth/client-secret` ref containing the client secret, when required. |
...
```bash
agh mcp authorize remote-docs
agh mcp authorize remote-docs --manual
agh mcp auth login remote-docs
agh mcp auth status remote-docs
agh mcp auth logout remote-docs

agh mcp authorize remote-docs --scope workspace --workspace <workspace-id>
```

`authorize` and `auth login` use the same daemon-owned PKCE session. Both print the live
authorization URL. `--manual` accepts either the returned code or the full redirect URL, which is
the supported path for remote operators and non-loopback HTTP binds. A command succeeds only after
status confirms both `authenticated` and `token_present`.

Agents can read the same redacted status from inside a session through `agh__mcp_status` or
...
runtime as `dead`. Settings, status, doctor, and `agh__mcp_status` expose the redacted diagnostic;
last-known tool descriptors remain visible through `agh__tool_info` with reason `backend_dead`
during the same daemon lifetime instead of disappearing from the catalog. AGH suppresses ordinary
...
| `models.curated[].cost_input_per_million`       | number  | empty.                             | Non-negative number.                                                                   | Input rate used for catalog-based usage estimates when the agent reports no actual cost.                               |
| `models.curated[].cost_output_per_million`      | number  | empty.                             | Non-negative number.                                                                   | Output rate used for catalog-based usage estimates when the agent reports no actual cost.                              |
...
Discovery adapters use the resolved provider auth, env, and home policy. Discovery never creates an
ACP session; if discovery is unavailable or fails, the catalog records source status and falls back
to stale or lower-priority rows. Session creation never depends on a successful discovery refresh.

`OpenClaw`, `Hermes`, and `Pi` only register a live provider source when `models.discovery.enabled
= true` and either `command` or `endpoint` is set; with no discovery wiring, those providers stay
...
`models.reasoning.apply = "none"` means AGH does not apply a reasoning level for the provider; it
...
Selectable effort lists come only from config/catalog/live truth. AGH does not infer them from GPT
...
AGH fetches the curated MCP server, extension, and skill documents from this base URL. Each kind is
validated independently and projected into the global database. If a fetch fails after a successful
projection, AGH keeps serving the prior rows with stale status instead of replacing them with an
...
`$AGH_HOME/config.toml`; workspace overlays and workspace-scoped CLI/native writes are rejected.

| Key        | Type     | Default                                                      | Validation                  | Effect                                                                             |
| ---------- | -------- | ------------------------------------------------------------ | --------------------------- | ---------------------------------------------------------------------------------- |
| `base_url` | URL      | `https://raw.githubusercontent.com/compozy/agh/main/catalog` | Absolute `http` or `https`. | Base directory for `mcp.json`, `extensions.json`, and `skills.json`; applies live. |
| `ttl`      | duration | `1h`                                                         | Positive Go duration.       | Age after which the next browse refreshes that kind; applies live.                 |
| `timeout`  | duration | `10s`                                                        | Positive Go duration.       | Deadline for one feed request; applies live.                                       |

```toml
[marketplace.catalog]
base_url = "https://catalog.example.com/agh"
ttl = "30m"
timeout = "5s"
```

These paths are `live` in the generated [lifecycle matrix](/runtime/core/configuration/lifecycle-matrix):
...
Agent-side config mutation is narrower than the operator CLI. `agh__config_set` accepts
...
`agh config set`. Both surfaces require global scope for every `marketplace.catalog.*` write.
...
When enabled, AGH redacts matching content before appending it to the global event ledger or a
...
Manage the setting through General Settings, `agh config set redact.enabled <true|false> -o json`,
or the live `agh__config_set` descriptor. These writes return
...
fallback (see [Extensions](/runtime/core/extensions)).
...
`allow_unverified` controls non-curated side-loads only. Enabling it permits the install path, but each install must still include the `--allow-unverified` confirmation. Curated installs always verify the catalog-pinned archive digest before extraction; a mismatch cannot be bypassed. Policy changes apply live.
...
| `allow_unverified` | boolean    | `false` | `true` or `false`                                   | Allows individually confirmed non-curated extension side-loads.    |

`extensions.marketplace.allow_unverified` is the only live path in this key family; the registry
and base URL remain restart-required under the broader `extensions.*` lifecycle. Verify the exact
result through `agh config reload -o json` and `agh config apply-history -o json`. A policy-blocked
side-load reports `extension_unverified_policy_blocked` and points to `/settings/extensions`.
...
value with `agh config set automation.suggestions.pending_cap <value>` and inspect the resulting
...
See [Vault](/runtime/core/configuration/vault) for the CLI, HTTP, UDS, web, and session-scoped
surfaces that store and inspect encrypted AGH-managed secret refs.
...
Compile-time ceilings stay in code. Operator config can tighten fan-out, no-progress, and gate
revision defaults, but it cannot loosen them past the hard runtime ceilings.

| Field                   | Delivery default | Watch default | Valid values              | Description                                                                 |
| ----------------------- | ---------------- | ------------- | ------------------------- | --------------------------------------------------------------------------- |
| `iteration_cap`         | `50`             | `0`           | Zero or positive integer. | Maximum loop iterations seeded for new runs. `0` means no configured cap.   |
| `no_progress.window`    | `3`              | `2`           | `0` through `30`.         | Generation-hash no-progress window. This is not the inactivity clock.       |
| `gates.max_revisions`   | `10`             | `0`           | `0` through `64`.         | Gate revision seed default. `0` means no gate-revision seed for that class. |
| `budget.tokens`         | `0`              | `0`           | Zero or positive integer. | Token budget. `0` means unlimited. A set budget is enforced.                |
| `budget.wall_clock_sec` | `0`              | `0`           | Zero or positive integer. | Wall-clock budget in seconds. `0` means unlimited.                          |
| `budget.on_exceeded`    | `halt`           | `halt`        | `halt` or `escalate`.     | Enforcement policy once a non-zero budget is exceeded.                      |
| `model_defaults.worker` | empty            | empty         | string.                   | Default model for `run-agent` actions that omit `params.model`.             |
| `model_defaults.judge`  | empty            | empty         | string.                   | Default model for `agent-judge` criteria that omit `model`.                 |
| `fan_out_width`         | `4`              | `2`           | `0` through `64`.         | Fan-out width seed default for new runs.                                    |

These paths are mutable through `agh config set` and the native config tools. Writes return
...
`agh config set` and the native config tools. Writes return `lifecycle="restart-required"`;
...
`$AGH_HOME/config.toml`, then the resolved workspace's `.agh/config.toml`. The Run start resolves
...
agh config set goals.max_turns 30 -o json

# Workspace desired state
agh config set goals.context_nudge_ratio 0.75 \
  --scope workspace \
  --workspace /absolute/path/to/workspace \
  -o json
```

The equivalent agent path is `agh__config_set` after resolving its live descriptor. Both surfaces
...
| Field                    | Type    | Default | Valid values          | Description                                                                                                                                                                         |
| ------------------------ | ------- | ------- | --------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `block_recurrence_limit` | integer | `2`     | `0` or a positive int | Same-kind re-block count that escalates a task to `needs_attention`. `0` disables the [unblock-loop breaker](/runtime/core/autonomy/task-runs-and-leases#the-unblock-loop-breaker). |
...
run or spawn a coordinator. Coordinator bootstrap is independent from Network participation: a
Local run can use the coordinator, and a Live channel is not a bootstrap precondition.
...
| `network_status_timeout`        | duration | `5s`    | Positive Go duration with whole-second precision.                     | Per-event timeout for summarizing task run status back to the AGH Network origin thread.                                                                            |
...
[Task Execution Profiles](/runtime/core/autonomy/execution-profiles) for the runtime behavior.
...
[Task Runs and Leases](/runtime/core/autonomy/task-runs-and-leases#force-operations). The setting
...
`agh network coordination status|enable|disable` or the equivalent HTTP/UDS surface.
...
- [Providers](/runtime/core/agents/providers) covers `[providers.<id>.models]` shape and `models.discovery` defaults.
- [Provider Model Catalog](/runtime/core/agents/model-catalog) explains the catalog projection, OpenAI-compatible endpoint, and extension `model.source`.
