...
# Concurrent autonomy-managed sessions (coordinator + spawned workers) per workspace.
...
| `max_active_sessions_per_workspace` | integer  | `5`           | `>= max_active_per_workspace`.            | Caps concurrent autonomy-managed sessions (coordinator + spawned workers) per workspace. Separate from coordinator uniqueness, which `max_active_per_workspace` plus the daemon singleton check enforce. |
