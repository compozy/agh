...
| `greet_interval`  | integer seconds | `30`    | Positive integer seconds. | Local peer-presence cadence.                                                                 |
| `max_replay_age`  | integer seconds | `300`   | Positive integer seconds. | Replay/freshness window for accepted envelopes.                                              |
| `max_queue_depth` | integer         | `100`   | Positive integer.         | Per-session accepted-delivery safety bound; it is not a participation or wake-count default. |
