...
| `[goals]`                            | Global/workspace defaults for newly started Goals.                              | 20 turns, context nudge ratio 0.8                                                                |
...
`[goals]` defines global or workspace defaults for newly started Goals. Both paths are mutable
through `agh config set` and the native config tools. Writes return
`lifecycle="restart-required"`; restarting activates the desired values for later starts without
rewriting active Runs or their checkpoints.
