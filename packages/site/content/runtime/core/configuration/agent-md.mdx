...
| `model`         | string                      | Provider `models.default`                   | Any string.                                                         | Agent-specific model metadata.                                            |
...
runtime capabilities by resolving canonical `agh__tool_search`, inspect descriptors through
canonical `agh__tool_info`, and invoke the harness-returned dedicated tool reference before
shelling out to equivalent `agh ...` commands. Operator-only management flows — daemon lifecycle,
MCP OAuth login/logout, raw secret writes, and trust-root config — remain CLI/HTTP/UDS by design and
are not promoted into the tool surface.
