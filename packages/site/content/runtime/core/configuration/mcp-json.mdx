...
| `env`             | string map   | empty               | String keys and values on `stdio` only.                 | Literal environment values. AGH does not expand `$VAR`, `~`, or shell syntax.  |
| `secret_env`      | string map   | empty               | `env:NAME` or `vault:mcp/**` refs on `stdio` only.      | Secret bindings resolved at launch. Settings reads expose refs, never values.  |
...
untouched. If Vault cleanup fails, AGH restores the prior MCP definition and owned secret.
...
over HTTP and UDS. `GET /api/mcp/oauth/callback` is HTTP-only and refuses automatic completion when
the daemon bind host is not loopback. On loopback, begin derives the callback origin from the
effective listener, including bracketed IPv6 addresses; an unavailable callback runtime returns a
documented `503` HTML response. Exchange accepts exactly one of `code` or `redirect_url`, and success
requires redacted status with both `authenticated` and `token_present`.
...
Use `--set KEY=VALUE` for non-secret catalog fields. For secret fields, create the value with
`agh vault put ... --value-stdin`, then use `--vault-ref`; this keeps secret material out of shell
arguments and history. Management reads return the bound `secret_env` refs so operators and agents
can identify which secret is connected without exposing its value.

## Inspect Runtime Status

The `/mcp` web page and `GET /api/settings/mcp-servers` keep configuration, authorization, runtime,
and probe state separate. A configured server is not automatically ready.
...
Open `/mcp?scope=global&server=<name>` to select a global definition. Workspace scope uses the
active workspace and `/mcp?scope=workspace&server=<name>`. The editor supports:
