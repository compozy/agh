...
dimension must exist on an inbound event. Google Chat emits `peer_id` plus an encoded `thread_id` for
DMs, and `group_id` plus an optional `thread_id` for spaces. It never emits peer and group together.
...
`--include-thread`, then repeat the same verification from a direct conversation. The space-wide
