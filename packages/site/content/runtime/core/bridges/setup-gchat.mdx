...
6. Verify identity/configuration, then enable and verify public reachability.
7. Install the app for a test user/space and send one inbound message.
8. Inspect the route and run a real `send-test` to its `spaces/...` target.
...
<OperatorNote>
  Service-account JSON is passed to the current generic CLI through `--secret-value`, which is a
  process argument. Use a short-lived environment variable only on a trusted host and clear it after
  the write. File/stdin input is not currently exposed; see [Bridge
...
- The route contains the expected `spaces/...` resource and thread identity.
