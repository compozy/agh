...
Direct DMs are evaluated by `dm_policy` first. Any admitted direct DM still fails the wizard's
default group-only route because it emits `peer_id`, not `group_id`; a direct-DM instance must use
`include_peer=true`. Use a separate callback boundary if server traffic also needs independent
sessions. Group DMs emit `group_id`, are not evaluated as direct DMs by this policy, and follow the
default group route instead.
