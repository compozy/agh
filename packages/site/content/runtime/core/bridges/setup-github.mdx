...
```bash
agh bridge secret-bindings put "$BRIDGE_ID" token \
  --secret-ref "vault:bridges/$BRIDGE_ID/token" \
  --kind token \
  --secret-value "$GITHUB_TOKEN"
```
...
agh bridge secret-bindings put "$BRIDGE_ID" app_id \
  ...
  --kind credential \
  --secret-value "$GITHUB_APP_ID"

agh bridge secret-bindings put "$BRIDGE_ID" private_key \
  --secret-ref "vault:bridges/$BRIDGE_ID/private_key" \
  --kind private-key \
  --secret-value "$GITHUB_APP_PRIVATE_KEY"
```

The App ID must be numeric. The private key must be an RSA key encoded as PKCS#1 or PKCS#8 PEM.

<OperatorNote>
  PATs and PEM private keys are passed to the current generic CLI through `--secret-value`, which is
  a process argument. Expand short-lived environment variables only on a trusted host. File/stdin
  input is not currently exposed; see [Bridge
