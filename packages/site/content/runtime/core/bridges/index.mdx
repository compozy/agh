...
| Secret binding     | AGH daemon          | Maps a provider slot such as `bot_token` to a daemon-resolved secret reference. The stock resolver supports `env:NAME`.            |
...
Bind each slot to an environment-backed secret before enabling the bridge:
...
The environment variable must exist and be non-empty in the daemon process. Other reference schemes
such as `vault://...` are not supported by the stock resolver.
