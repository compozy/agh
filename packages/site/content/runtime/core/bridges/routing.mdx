...
acknowledged sequence and remote-message identity at registration, material acknowledgement, and
terminal transitions. Streamed delta text and tool-progress payloads are not stored in this ledger.
Rows and durable metrics carry direct scope and workspace ownership, so startup reconciliation never
lists or handles another workspace's delivery.

At daemon startup, AGH reconciles unfinished rows before the broker admits new registrations or
inbound prompt side effects. Because the checkpoint-only ledger does not retain response text, each
unfinished row fails open as a new visible terminal `error` post with the standard session-stopped
message. This works for append-only providers and does not depend on an old remote message still
being editable. When a live session stops normally, AGH likewise sends terminal `error` delivery
events for unfinished responses before the bridge notifier finishes the stop path.
...
responses. For live monitoring, subscribe to `GET /api/bridges/health/stream` with the required
`bridge_ids` query parameter. Pass the comma-separated IDs from the current catalog page (up to
200), and reuse that page's `scope` and `workspace_id` query values. The stream emits an initial
snapshot and then changes only for that authorized, page-bounded set; loading another catalog page
opens a stream for its IDs instead of hydrating the full registry.
