...
The provider's default DM policy is `open`. Before exposing a broadly reachable business number,
edit the disabled bridge in the Web UI and select `allowlist` or `pairing`, then add WhatsApp user IDs
under `provider_config.dm.allow_user_ids` or `paired_user_ids`.

WhatsApp user IDs are phone numbers with country code and no `+`, spaces, or punctuation, for example
`15551234567`. A username/display-name rule is more fragile than an ID rule.
...
agh bridge secret-bindings put "$BRIDGE_ID" access_token \
  --secret-ref "vault:bridges/$BRIDGE_ID/access_token" \
  --kind token \
  --secret-value "$NEW_WHATSAPP_ACCESS_TOKEN"

agh bridge enable "$BRIDGE_ID"
agh bridge verify "$BRIDGE_ID" --json
```

The guided wizard uses hidden input for initial setup. The generic binding command receives
`--secret-value` as a process argument; see the client-side exposure note in
