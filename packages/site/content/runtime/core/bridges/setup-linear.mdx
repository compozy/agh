...
agh bridge secret-bindings put "$BRIDGE_ID" webhook_secret \
  ...
  --kind secret \
  --secret-value "$LINEAR_WEBHOOK_SECRET"
```

API-key authentication:

```bash
agh bridge secret-bindings put "$BRIDGE_ID" api_key \
  --secret-ref "vault:bridges/$BRIDGE_ID/api_key" \
  --kind token \
  --secret-value "$LINEAR_API_KEY"
```

OAuth client credentials:

```bash
agh bridge secret-bindings put "$BRIDGE_ID" client_id \
  --secret-ref "vault:bridges/$BRIDGE_ID/client_id" \
  --kind credential \
  --secret-value "$LINEAR_CLIENT_ID"

agh bridge secret-bindings put "$BRIDGE_ID" client_secret \
  --secret-ref "vault:bridges/$BRIDGE_ID/client_secret" \
  --kind secret \
  --secret-value "$LINEAR_CLIENT_SECRET"
```

<OperatorNote>
  API keys and OAuth client secrets are passed to the current generic CLI through `--secret-value`,
  which is a process argument. Expand short-lived environment variables only on a trusted host.
  File/stdin input is not currently exposed; see [Bridge
...
Use that thread in a real delivery:
...
In comments mode, the response appears as a Linear comment. In Agent Session mode, it appears as an
