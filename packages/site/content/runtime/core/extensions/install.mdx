...
extensions are constrained to a read-oriented ceiling: `logs.read`, `memory.read`, `observe.read`,
`session.read`, `skills.read`, and `tool.read`. Host API actions are filtered to methods whose
required capability fits that ceiling.
