...
dates remain absent rather than being inferred.
...
Extension source rows can carry `deprecated`, `hidden`, `featured`, and nullable `release_date`
metadata. They are always validated through `internal/modelcatalog`; invalid dates or other malformed
rows produce a recorded source status (with redacted error) instead of corrupting the projection.
