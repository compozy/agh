...
5. AGH Network `greet` presence is independent. A peer being online does not make a session
   wake-eligible.
...
- `HEARTBEAT.md` is **not** AGH Network presence. Peer presence and `greet_interval` belong to
  the [protocol model](/runtime/core/network/protocol). Network presence cannot make a session
  wake-eligible.
