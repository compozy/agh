...
switches to active ACP `configOptions` for live model and reasoning controls. Agents that do not
advertise a model config option keep the launch/default model when no model override is requested.
If a session requests a model override and the agent does not advertise a model config option,
session start or resume fails with an explicit error; AGH does not call a legacy model-switch RPC.
The catalog informs pre-session selection, while live ACP options govern controls inside the
running session.
