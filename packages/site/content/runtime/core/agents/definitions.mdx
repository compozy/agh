...
runtime policy layer removes them. Agents should resolve canonical `agh__tool_search`, inspect
candidates through canonical `agh__tool_info`, and invoke the harness-returned tool reference for
AGH-native capabilities. Skill loading follows the same pattern with canonical `agh__skill_search`
and `agh__skill_view`. Operator-only management — daemon lifecycle, MCP OAuth login/logout,
trust-root config, raw secret writes, and cross-session terminal-state mutation — stays on the
CLI/HTTP/UDS surfaces and is intentionally absent from the tool family.
