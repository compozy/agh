...
| ------------------ | -------------- | ---------------------------------------------------------------------------------------------------------------- |
| `surface:"thread"` | `thread_id`    | Public N-to-N conversation inside one channel. Visible to every channel member.                                  |
| `surface:"direct"` | `direct_id`    | Restricted two-party conversation inside one channel. Visible to the two room peers and the daemon's audit path. |

`thread_id` and `direct_id` are mutually exclusive on a single envelope. Setting both is a
validation failure.

`direct_id` is deterministic: the runtime derives it from `(channel, sorted(peer_a, peer_b))` with a
domain-separated SHA-256 hash. Any peer pair maps to exactly one direct room per channel. Two
sessions opening the same direct room observe the same `direct_id`.
...
6. Apply routing rules for `to`, `channel`, conversation container, and the local peer set.
