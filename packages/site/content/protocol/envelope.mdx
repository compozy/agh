...
description: Reference for the AGH Network v2 JSON envelope, conversation surface fields, work lifecycle correlation, validation order, schema, and annotated examples.
icon: FileText
---
...
| `proof`        | object or null | Reserved    | MAY be `null` in v2. v2 receivers MUST NOT require cryptographic proof processing.                                                                                                                                     |
...
```json
{
  ...
  "$id": "https://agh.dev/schemas/network/v2/envelope.schema.json",
  "title": "AGH Network v2 Envelope",
  "type": "object",
  "additionalProperties": false,
  "required": ["protocol", "id", "workspace_id", "kind", "channel", "from", "ts", "body"],
  "properties": {
    "protocol": {
      "const": "agh-network/v2"
    },
    "id": {
      "type": "string",
      "minLength": 1
...
| Field          | Annotation                                                                             |
| -------------- | -------------------------------------------------------------------------------------- |
| `protocol`     | Selects the v2 protocol profile.                                                       |
| `id`           | Identifies this envelope for deduplication and correlation.                            |
| `kind`         | `say` is the default conversation kind in v2.                                          |
...
| `proof`        | Reserved for trust profiles; null in v2.                                               |
