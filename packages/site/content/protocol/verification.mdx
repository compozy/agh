...
<OperatorNote label="Implementation status">
  The current AGH Runtime implementation validates v2 envelopes and routes opaque `proof` values.
...
| --------------------------------------------------------------------------------------- | ------------ | ------------------------------------------------------------------ |
| `proof` absent or `null`, `from` is a normal v2 Peer ID                                 | `unverified` | Receiver MAY route under unverified policy.                        |
...
- It MUST NOT treat transport authentication, NATS account membership, or Peer Card claims as a
