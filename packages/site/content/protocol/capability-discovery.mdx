...
- A `capability` envelope MUST carry a conversation surface (`surface:"thread"` with `thread_id`,
  or `surface:"direct"` with `direct_id`) and MAY carry a `work_id`. When `work_id` is present,
  the capability participates in the same lifecycle rules as a `say` envelope with `work_id`.
...
<OperatorNote>
  Capability claims remain advisory in v2. `agh.capability_catalog` is a richer hint, not a contract
