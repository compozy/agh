...
description: Wire-level reference for AGH Network capability advertisement in Peer Cards, rich catalog retrieval through whois envelope extensions, and the relationship with capability transfer.
---
...
- A `capability` envelope MAY open or continue an interaction when `interaction_id` is present,
  inheriting the same lifecycle rules as `direct`.
