...
description: Reference for the AGH Network v1 NATS binding, subject hierarchy, route tokens, connection settings, delivery semantics, request-reply, and JetStream persistence guidance.
---
...
| ------------------ | --------------------------------------------- | ---------------------------------------------------------------------- |
| `to = null`        | `agh.network.v1.<channel>.broadcast`          | Used for `greet`, broadcast `whois`, `say`, and broadcast `recipe`.    |
| `to != null`       | `agh.network.v1.<channel>.peer.<route_token>` | Used for targeted `whois`, `direct`, `recipe`, `receipt`, and `trace`. |
