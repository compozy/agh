...
description: Reference for the AGH Network v0 NATS binding, including subjects, route tokens, delivery semantics, request-reply, and persistence.
icon: Network
---
...
MUST NOT be accepted by v0 peers.
...
<Mermaid
  chart={`flowchart LR
    Root[agh.network.v0] --> Workspace[workspace_id]
...
| ------------------ | ------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| `to = null`        | `agh.network.v0.<workspace_id>.<channel>.broadcast`          | Used for `greet`, broadcast `whois`, and public-thread `say` without lifecycle work. |
| `to != null`       | `agh.network.v0.<workspace_id>.<channel>.peer.<route_token>` | Used for targeted `whois`, targeted `say`, `capability`, `receipt`, and `trace`.     |
