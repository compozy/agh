...
description: Reference for the AGH Network v1 NATS binding, including subjects, route tokens, delivery semantics, request-reply, and persistence.
icon: Network
---
...
<OperatorNote label="Implementation status">
  The current AGH Runtime embeds NATS Core for v0 with subject prefix `agh.network.v0`. It does not
  currently expose the v1 prefix, verified route tokens, external NATS configuration, or JetStream
  persistence as a public interoperability surface.
</OperatorNote>
...
<Mermaid
  chart={`flowchart LR
    Root[agh.network.v1] --> Channel[channel]
...
| ------------------ | --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `to = null`        | `agh.network.v1.<channel>.broadcast`          | Used for `greet`, broadcast `whois`, public-thread `say`, and broadcast `capability`.                                |
| `to != null`       | `agh.network.v1.<channel>.peer.<route_token>` | Used for targeted `whois`, targeted `say` (thread or direct surface), targeted `capability`, `receipt`, and `trace`. |
...
The v1 NATS prefix prevents ambiguous routing between untrusted v0 identities and verified-mode v1
identities. A bridge between prefixes MUST preserve envelope semantics and MUST NOT rewrite proofs
unless it re-signs as a new sender.
...
Receivers MUST verify v1 proofs according to [signature verification](/protocol/verification/)
