...
<OperatorNote label="Implementation status">
  RFC 004 defines this v1 trust profile. The current AGH Runtime `internal/network` package is a v2
  implementation: it preserves `proof` as opaque JSON, uses `agh-network/v2`, and does not yet
...
```json
{
  ...
  "body": {
    "peer_card": {
      ...
      "profiles_supported": ["agh-network/v2", "agh-network.trust.ed25519-jcs/v1"],
