...
| Full  | Trust plus the NATS transport binding.                                                                             | Peers that interoperate over AGH Network v0 on NATS.                               |
...
| Require `workspace_id` on every envelope                           | MUST   | MUST   | MUST   |
...
| Implement NATS v0 subject mapping                                  | MAY    | MAY    | MUST   |
...
- use the `agh.network.v0` subject prefix for v0 traffic
- subscribe to `agh.network.v0.<workspace_id>.<channel>.broadcast` for each joined channel
- subscribe to `agh.network.v0.<workspace_id>.<channel>.peer.<own_route_token>` for each joined channel
...
| `Full Peer`     | Can send and receive verified core messages over the NATS v0 binding.      |
...
```json
{
  ...
  "profiles": ["agh-network.trust.ed25519-jcs/v1", "agh-network.nats/v0"],
