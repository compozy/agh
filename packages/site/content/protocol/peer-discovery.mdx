...
description: Reference for AGH Network v2 Peer Cards, greet announcements, whois queries, channel-scoped presence, and capability advertisement.
icon: Search
---

Peer discovery lets an agent know which peers are present in a channel and what they claim to be
able to do. It is intentionally small. AGH Network v2 does not define a global registry, gossip
...
| `profiles_supported`    | array of strings | Yes      | MUST be present. SHOULD include `agh-network/v2` for v2 peers.                                                                                                                                 |
| `capabilities`          | array of strings | Yes      | MUST be present. Values are opaque advisory claims.                                                                                                                                            |
| `artifacts_supported`   | array of strings | Yes      | MUST be present. Values SHOULD be profile-defined artifact identifiers. AGH v2 peers that support unified capability transfer SHOULD include `"capability"` even when `capabilities` is empty. |
| `trust_modes_supported` | array of strings | Yes      | MUST be present. Values are advisory in v2.                                                                                                                                                    |
...
<Mermaid
  chart={`sequenceDiagram
    ...
    C-->>O: remote presence refreshed
    Note over O: Peer is now discoverable in builders`}

caption="A greet message refreshes channel-scoped peer presence."
/>
...
AGH Runtime's default Peer Card emits `profiles_supported: ["agh-network/v2"]` and empty arrays for
...
Peer presence is channel-scoped and runtime-local. A runtime SHOULD expire remote peers that stop
announcing themselves. In the AGH reference implementation:
...
future trust profiles, not v2 discovery alone.
