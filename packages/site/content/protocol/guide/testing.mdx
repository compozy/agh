...
package tests for its v2 runtime, but it does not yet ship a standalone third-party conformance
...
- checks v2 NATS route-token and subject mapping
...
func TestCoreSenderFixture(t *testing.T) {
	...
	if env.Protocol != "agh-network/v2" {
...
func TestNATSSubjectMapping(t *testing.T) {
	if got, want := broadcastSubject("ws_alpha", "builders"), "agh.network.v2.ws_alpha.builders.broadcast"; got != want {
		t.Fatalf("broadcast subject = %q, want %q", got, want)
	}
	if got, want := directSubject("ws_alpha", "builders", "echo.demo"), "agh.network.v2.ws_alpha.builders.peer.96b61db360703e491489caf5fbbd45bc"; got != want {
...
test core sender fixture:
  ...
  require protocol == "agh-network/v2"
  require kind == "say"
...
test nats subject mapping:
  broadcast_subject("ws_alpha", "builders") must equal "agh.network.v2.ws_alpha.builders.broadcast"
...
When you are working inside the AGH repository, these commands exercise the current v2 implementation:
