...
description: Complete AGH Network v2 examples for public thread coordination, restricted direct-room handoff, peer discovery, and capability transfer.
icon: FileCode
---
...
The control plane derives the deterministic `direct_id` from `(channel, sorted(peer_a, peer_b))`.
The wire envelope still carries the explicit `direct_id`.
...
### 5. Release bot opens lifecycle-bearing work in the same thread
