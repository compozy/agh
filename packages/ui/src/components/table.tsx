...
function TableFooter({ className, ...props }: React.ComponentProps<"tfoot">) {
  return (
    <tfoot
      ...
      className={cn(
        "border-t border-(--line) bg-(--canvas-tint) text-[13px] text-(--fg) font-[510] [&>tr]:last:border-b-0",
        className
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-9 px-3 text-left align-middle font-mono text-[10.5px] font-[510] uppercase tracking-(--tracking-mono) whitespace-nowrap text-(--muted) [&:has([role=checkbox])]:pr-0",
        className
      )}
      {...props}
    />
