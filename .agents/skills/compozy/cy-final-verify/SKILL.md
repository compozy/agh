...
Before collecting completion evidence, run the `deslop` skill on the branch diff and apply its cleanup. Order matters: deslop mutates code, so verification evidence gathered before it is stale — deslop first, then verify. Skipping it ships AI slop (noise comments, defensive clutter, style drift) into permanent artifacts.
...
**Before `git commit`:**
1. Run the `deslop` pass (see "Deslop Gate" above).
...
4. If the project keeps a living QA tracker (e.g. `docs/qa/state.csv`), apply the QA impact flag (see "QA Tracker Impact" below).
...
A green pipeline proves the code works; it does not keep QA verdicts honest. When the project keeps a living QA tracker (e.g. `docs/qa/state.csv`), a completion claim also requires the impact flag — one question, ~1 minute:
...
**Flag, don't retest.** Running QA is the QA cycle's job — `untested` rows are exactly its scope. Skipping the flag silently is a stale-verdict claim: the same dishonesty as claiming tests pass without running them.
