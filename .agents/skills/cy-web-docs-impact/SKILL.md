...
3. Append a `QA impact` line to the `### Web/Docs Impact` subsection: when the task changes user-visible behavior (UI, CLI verb, API route, config key, copy), list the `docs/qa/state.csv` scenario ids the executor must reset to `untested` at completion (or `new scenarios — add untested rows at completion`); otherwise write `QA impact: none — no user-visible behavior change`. Flag, don't retest — retests belong to the next QA cycle.
