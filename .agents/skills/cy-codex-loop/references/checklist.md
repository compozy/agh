...
- [ ] The dispatched `cy-*` skills for the printed phase were activated **before** any code edits or reviews.
- [ ] Memory was updated via `cy-workflow-memory` before any state-flipping operation (`cy-execute-task` step 5 sequence: memory → checkboxes → status → master → commit).
...
- [ ] `cy-final-verify` ran for any iteration that produced code or fixes; its output is cited in the iteration summary's `verify_evidence`.
