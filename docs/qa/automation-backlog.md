...
- Why automate: the truthful-outcome guarantee (no coercion) needs seeds that actually produce each of the 11 states — incl. `no_op`, `blocked` (ADR-022), `queued` (ADR-021), and `paused` — which today's daemon rarely emits richly. Pins the "never render a terminal coercion" invariant.
