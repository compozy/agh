...
An agent drives and reads sessions deterministically over any surface (`_qa.md` §3 J-E). A headless actor creates a session, sends a prompt, subscribes to the stream (snapshot replay or `Last-Event-ID` delta), reads bounded transcript/events/history/recap with cursor pages, stops the session, and confirms list/detail/status agree — CLI ↔ HTTP ↔ UDS parity is the point (RT-042 canary). The bugs live in unbounded reads, non-deterministic snapshot replay, keep-alive gaps, stop-race recorder errors, and lifecycle desync.
...
flowchart TD
    ...
    C --> STRM[Subscribe SSE: snapshot replay or Last-Event-ID delta]
    STRM -->|idle active session, stale cursor| SNAP[Non-empty snapshot seed]
    STRM -->|idle| KA[Keep-alive comment frames ≤ heartbeat interval]
    C --> RD[Read transcript/events/history: bounded defaults + cursor pages]
    ...
    STRM -.->|client killed mid-stream| AB[Abandon: reconnect after_sequence — gap-free resume]
    AB -.-> STRM
...
journey:
  ...
  value_statement: "An agent can drive and read sessions deterministically over CLI, HTTP, or UDS — bounded reads, stable cursors, snapshot replay, keep-alive, gap-free reconnect, and identical lifecycle state everywhere."
  personas: [Ada]
  entry_points:
  ...
  actions:
    ...
    - step: 2
      ...
      expected_observable: "Resubscribe with a stale `Last-Event-ID` (or `?replay=snapshot`) against an idle active session yields a non-empty snapshot seed; idle streams emit keep-alive comment frames ≤ heartbeat; transcript/events/history/recap return bounded defaults with stable cursor pages"
    - step: 3
  ...
  true_end_state: "Reconnect after killing the client mid-stream resumes gap-free from `after_sequence`; list/detail/status still agree after a daemon restart; the same verb over HTTP and UDS returns identical structured output."
  exit:
  ...
  abandonment:
    - at_step: 2
      ...
      resume: "Reconnect with `after_sequence` / `Last-Event-ID` resumes gap-free with no skipped `sequence`; a snapshot seeds an idle session so the resume is never silently empty."
    - at_step: 3
...
design_reference:
  ...
  truthful_ui_checks:
    ...
    - "Snapshot-on-resubscribe seeds an idle active session — the stream is never silently empty on reconnect (task 17)."
    - "Bounded read defaults + stable cursor pages; no unbounded transcript replay (task 14/15)."
...
e2e_backbone:
  runtime:
    - "E2E-runtime 1: long-session read path under lane latency with the snapshot cache (task 14)."
