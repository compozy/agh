...
```mermaid
flowchart TD
  A[Entry: Marketplace sidebar item mid-session] --> B[Landing: one cards-first section per kind, real items at idle]
  A2[Entry: deep link /marketplace/$kind/$entryId] --> D
  A3[Entry: docs guide agh.network/runtime/core/marketplace] --> B
  B --> C[One search fans out to all kinds, grouped]
  C -->|one source fails| C2[Failed kind owns its error strip; siblings usable]
  C2 --> C
  B --> D[Inspect stable entry detail by entry_id]
  C --> D
  D --> E{Entry kind}
  E -->|Skill| F[One-click Install or Update]
  F --> F2[True end skill: card flips installed; Manage opens /skills/$name; timer < 60s]
  ...
  G1 --> G4[True end extension: installed with truthful provenance; Manage opens /extensions/$name; timer < 60s]
  G3 --> G4
  ...
  H3 --> H4[True end bundle: activation visible at /extensions?tab=bundles and detail; timer < 60s]
  ...
  I3 --> I5[True end MCP: server on /mcp with truthful readiness state, never false-ready; timer < 60s]
...
journey:
  ...
  actions:
    - step: 1
      verb: Detour mid-session to Marketplace and browse or search the grouped catalog
      expected_observable: Real items per kind at idle; healthy kinds remain usable when one source fails; counts and empty states stay truthful
    - step: 2
    ...
    - step: 4
      ...
      expected_observable: The management home opens preselected on the exact acquired capability (/skills/$name, /extensions/$name, /extensions?tab=bundles, /mcp?scope=&server=)
    - step: 5
...
- Taxonomy sweep: journeys (per-kind happy paths above), functional (validation, entry-id identity, round-trips — scenario rows), experiential (keyboard operability with open `BUG-20260714-keyboard-focus-invisible`; curated-showcase copy must not imply exhaustiveness), edge/error/empty (partial-failure strips, all-zero search, abandonments above), cross-cutting (mobile widths on landing rows; regression canary is CH-033 on J-22).
