...
persona: Dora
journey:
expected: Opening Create agent shows one surface, never a stepper — no "Step N of 4", no Back/Continue. Simple carries the definition (agent name, instructions), the runtime selector with live catalog state, and the permission policy as cards. Advanced adds runtime overrides (runtime command, category path as a single mono input) and the tool/skill allowlists, without hiding any Simple field. The primary stays live while the visible tier is valid; submitting with an invalid advanced-only field (a category path such as `operations//incident`) reveals Advanced and shows the field error instead of discarding what was typed. Leaving Advanced preserves every authored value. There is no MCP servers control anywhere in the dialog, and the created request never carries `mcp_servers`. The category path is sent as `category_path` segments split on `/`. Scope sits in the toolbar and reports the workspace the definition lands in; global scope omits `workspace`.
