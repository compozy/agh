...
expected: The waiting dialog exposes a manual completion field that accepts a bare code OR a full redirect URL and sends exactly one value to `auth/exchange`. Success requires the refetched `authenticated && token_present`; a non-confirmed exchange leaves the dialog failed with the prior status intact. The live URL stays copyable throughout.
