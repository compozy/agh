...
expected: Startup stops before normal work, while local CLI opens return exactly one parseable JSON error document with diagnostic.code schema_ahead, the surface, canonical path, and newer-binary-or-complete-family remediation.
entry_points: agh daemon start --foreground; agh extension list -o json; agh mcp auth status -o json
qa_status: untested
bug_ids:
fix_status:
retest_status:
