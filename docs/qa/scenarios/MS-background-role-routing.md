...
entry_points: config.toml; agh config set roles.<role>.<key> <value>; agh config set --scope workspace --workspace <root> roles.<role>.<key> <value>; agh__config_list|get|set|unset over exact roles.* leaves (agh__config_path proves the selected scope target only); docs runtime/core/configuration/config-toml [roles]
qa_status: pass
