# Hermes & OpenClaw Hype Templates → AGH Adaptations
...
The first version of this file invented features. It claimed an "agh 0.0.5" release, a "background task panel with scroll position restore", Bitwarden Secrets Manager support, Krea 2 built-in, and specific PR-log items that don't exist. None of those are in the codebase. The latest tag is `v0.0.4` and main has 0 commits past it at the time of writing.

This rewrite anchors every AGH-side claim on a code reference. Adaptations that would require a feature AGH doesn't ship today are marked **TEMPLATE BLOCKED** with what would have to land first. Adaptations that hold up against the current code are marked **SHIPPABLE TODAY**.

## Verified AGH surface area (used as the only source of truth below)

| Primitive | Where | Notes |
|---|---|---|
| Hero hook | `COPY.md § 2 — Positioning Snapshot` | "An open workplace for AI agents." |
| One-liner | `COPY.md § 2` | "AGH is a local-first agent operating system: one daemon for durable agent sessions, one operator surface for humans and agents, and one open network for agent-to-agent coordination." |
...
| MCP integration | `internal/mcp/hosted.go:29–60+` | `agh-hosted-tools` session-scoped MCP service, nonce-authed bind, tool invocation via canonical ToolID registry |
...
| `agh-network/v0` | `internal/network/envelope.go:12` | 6 message kinds: greet, whois, say, capability, receipt, trace |
...
**TEMPLATE BLOCKED**: AGH integrates MCP via `agh-hosted-tools` (`internal/mcp/hosted.go`) but there is no user-facing **catalog** UI verified. Don't claim catalog parity until a discoverable catalog ships.

**SHIPPABLE TODAY** (different angle, same primitive):
> mcp inside agh is workspace-scoped, not agent-scoped.
>
> register the server once, every hosted agent in that workspace can call its tools through `agh-hosted-tools`. one nonce per session, no key juggling per agent.
...
**SHIPPABLE TODAY** — OpenRouter and Qwen are real providers in `internal/config/provider.go`. The CLI shape needs to be sanity-checked against the actual `agh agent` subcommands before posting (run `agh agent --help` and adjust):

> agh hosts qwen 3.7 max through openrouter today.
>
> rough setup (verify against `agh agent --help`):
> → install agh
...
**TEMPLATE BLOCKED**: Krea is not in the AGH codebase. `grep -ri krea` returns no matches. Don't fake parity.
...
- Or pivot to a verified provider that's already there (xAI / Grok image, if/when that lands in AGH; or a model that just got added to `provider.go`). Check `internal/config/provider.go` first.
...
**TEMPLATE BLOCKED**: Bitwarden is not in the AGH codebase. Don't fake the co-brand.

**Reframe**: AGH does have `internal/providerauth` and `bound_secret` / `native_cli` / `none` auth modes per `internal/config/provider.go`. If you want a secrets-management tweet, write it about that existing surface instead of a vendor partner:

> agh splits provider auth into three modes per provider: native_cli, bound_secret, none.
>
...
**SHIPPABLE TODAY** only if AGH actually runs an open-hour ritual. If there isn't one yet, this is a **TEMPLATE BLOCKED** that needs the ritual to exist first.

**Draft (assuming the ritual exists or you're starting one)**:
> agh open hour wednesday 6pm BRT.
>
...
**TEMPLATE BLOCKED** until AGH has both the recurring jam **and** a real headcount + a real demo to point to. Don't pre-fake either.

**When ready**:
> {real headcount} people in the agh open hour right now.
>
...
>
> what's your audit story when a peer card gets compromised? agh peer cards carry PeerID + Capabilities + TrustModesSupported, so revocation has surface area, but the operator playbook still feels open. open to wrong answers.
...
**TEMPLATE BLOCKED**: there is no public "workspace router" or similar extracted Go module from AGH today. Don't claim an extraction that hasn't happened. Use this template only after a real extraction lands.
...
> teams of agents running on one laptop, coordinating over `agh-network/v0`, closing work with receipts.
>
> agh ships the runtime piece. workspaces hold the state. ClaimNextRun moves work between agents. one go binary, no infrastructure to babysit. start with a fleet of two.

**Voice check**: format=`technical-insight`. Cites `agh-network/v0`, workspaces, ClaimNextRun — all verified. No em-dash. 0 emojis.
...
**Pattern note** for when there's a real story: lead with the outcome number, name the agents inside the AGH workspace, end with cost or time saved. Only ship when the project actually exists.
...
**TEMPLATE BLOCKED** until you've actually wired a voice loop through AGH. Don't pre-claim the build.
...
**SHIPPABLE TODAY** — AGH is a daemon designed for durable sessions per `COPY.md § 2`, so "still grinding" is honest:

> sunday night. agh daemon still grinding in the background. one go binary, sqlite-backed session state, my laptop, 168 hours of it if i want.
...
> ran grok 4.3 inside agh against both claude code and openclaw, same workspace.
>
...
**TEMPLATE BLOCKED**: don't post about competitor outages you didn't witness. If a real outage hits and the AGH workspace genuinely keeps routing tasks across surviving agents, that's a great tweet — but only then.

**Future-shippable shape (anchor on `ClaimNextRun` + hooks)**:
> {outage} hit today. {agent} died on auth. agh workspace kept routing claims to the surviving agents.
>
...
> okay folks on agh v0.0.4 running claude code as a hosted agent: are the lease heartbeats holding under long-running task runs?
>
> want signal before i open a fix. drop your `agh task` output if you've seen drops.

**Voice check**: format=`question`. Version is correct. `lease heartbeats` and `ClaimNextRun` lease TTL are real per `internal/task/lease_manager.go`. Verify `agh task` is the actual CLI noun (it might be `agh automation` or `agh runs` — check `internal/cli/automation.go`) before posting.
...
**TEMPLATE BLOCKED for a fake release**. The last AGH tag is `v0.0.4` with 0 commits since on `main`. Don't ship a release digest until a real `v0.0.5` tag exists.

**When ready, use this honest version-digest shape**:
> agh v0.0.{N} shipped. {real commit count} commits since v0.0.4.
>
> highlights:
...
>
> agh is the runtime piece. workspaces hold state. ClaimNextRun moves work. hooks observe everything. agh-network/v0 lets sessions become peers.
...
> shipped {N} PRs into agh main today:
>
...
- **Opener cooldown**: posts that start `agh + verb` ("agh hosts qwen…", "agh splits provider auth…", "agh open hour…") share an opener fingerprint shape — space them out by 48h.
...
- **Comparison tone**: drafts 14, 15, 17 mention OpenClaw / Hermes / Claude Code by name. Tone stays comparative and respectful — these are featured hosts inside AGH, not punching bags.
...
- **Pre-publish CLI verb check**: a handful of drafts reference CLI verbs (`agh agent`, `agh task`, `agh workspace`) without confirming the exact spelling. Before posting any draft that includes a command, run `agh --help` and replace placeholders with real verbs.
