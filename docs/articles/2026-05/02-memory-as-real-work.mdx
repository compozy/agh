...
*Three scopes feed one session view. Agent overrides workspace; workspace
overrides global. Higher-precedence entries shadow lower ones by name.*
...
The default *write* scope is not picked at random either. The runtime resolves
...
*Memory on disk: one Markdown file per entry, one index file per scope. The
index lives at `internal/memory/store.go:27` as `indexFilename = "MEMORY.md"`.*
...
The closed type set has a second job: it constrains the *write* side. A new
...
*The cascade is ordered by cost. The time gate is a single stat on the lock
...
*The same verbs work for the runtime, the operator, and another agent
