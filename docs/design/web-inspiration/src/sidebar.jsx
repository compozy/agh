...
function Sidebar({ route, setRoute, sessionId, setSessionId }) {
  ...
  const agents = [
    { key: 'claude', name: 'claude', provider: 'Anthropic', sessions: [
    ...
    { key: 'codex', name: 'codex', provider: 'OpenAI', sessions: [
      { id: 'x-1', name: 'perf audit', state: 'active' },
    ]},
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-panel">
        <div className="sidebar-head">
          ...
          <button className="btn-icon"><Icon.Search size={13} /></button>
          <button className="btn-icon"><Icon.PanelLeft size={13} /></button>
        </div>
        ...
        <div className="scroll-y" style={{ flex: 1 }}>
          ...
          {agents.map(a => (
            <div key={a.key}>
              <div className="agent-row" onClick={() => setExpanded(s => ({ ...s, [a.key]: !s[a.key] }))}>
                <span style={{ color: 'var(--color-text-tertiary)', display: 'inline-flex' }}>
                  {expanded[a.key] ? <Icon.ChevronDown size={12} /> : <Icon.ChevronRight size={12} />}
                </span>
              ...
              {expanded[a.key] && a.sessions.map(s => (
                <div key={s.id}
                     className={`session-row ${route === 'session' && sessionId === s.id ? 'active' : ''}`}
                     onClick={() => { setRoute('session'); setSessionId(s.id); }}>
                  <span className={`dot ${stateToDot(s.state)}`} />
                  <span className="truncate">{s.name}</span>
                </div>
              ))}
              {expanded[a.key] && a.sessions.length === 0 && (
                <div style={{ marginLeft: 22, padding: '3px 8px', fontSize: 11, color: 'var(--color-text-tertiary)' }}>
                  No sessions
                </div>
              )}
            </div>
          ...
          <div className="sidebar-section-label" style={{ marginTop: 12 }}>Workspace</div>
          {NAV_ITEMS.map(item => (
            <div key={item.to}
                 className={`nav-item ${route === item.to ? 'active' : ''}`}
                 onClick={() => setRoute(item.to)}>
              <item.icon size={14} style={{ color: route === item.to ? 'var(--color-text-primary)' : 'var(--color-text-tertiary)' }} />
        ...
        <div className="sidebar-footer">
          <div className="flex items-center gap-2" style={{ marginBottom: 6 }}>
            ...
            <span className="mono" style={{ fontSize: 10, color: 'var(--color-text-secondary)' }}>connected</span>
            <span className="mono" style={{ fontSize: 10, color: 'var(--color-text-tertiary)', marginLeft: 'auto' }}>v0.4.1</span>
          </div>
          <div className={`nav-item ${route === 'settings' ? 'active' : ''}`}
               style={{ margin: 0 }} onClick={() => setRoute('settings')}>
...
function AgentGlyph({ name }) {
  const colors = {
    claude: '#E8572A', codex: '#8E8E93', gemini: '#BF5AF2',
    opencode: '#30D158', cursor: '#E5E5E7',
  };
  return (
    <span style={{
