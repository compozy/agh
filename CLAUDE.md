...
- **ALWAYS USE** the `svelte5-best-practices` skill before writing or modifying any Svelte 5 code
...
- **Designed for incremental extension** — every new capability (memory, skills, agent networking) must arrive as a new package wired into `daemon/`, without modifying existing packages. Use small interfaces and dependency injection so future features plug in rather than fork existing code. Keep public APIs of each package stable — add, don't change.
...
- **Interfaces defined where consumed** (Go-style) — `session/` defines `AgentDriver`, `acp/` implements it without knowing
...
- **No back-pointers between packages** — if session/ needs something from daemon/, inject a callback or interface
- **Functional options for constructors** — `NewManager(opts ...Option)`
- **Maps for <10 items** — no registry interfaces for small collections
- **File-level organization** within packages — split into files, sub-packages only when complexity justifies it
...
The `.old_project/` directory contains the previous AGH implementation (78K+ LOC). It is **reference only** — do not modify, do not import, do not include in builds. Use it to understand concepts (memory, observability, config patterns) but never copy code directly. Go tooling ignores it (dot-prefix). When searching the codebase, **exclude `.old_project/`** from results to avoid confusion.
...
- **80% coverage minimum** per package. Run `go test -coverprofile=coverage.out ./...` and check with `go tool cover -func=coverage.out`.

### Integration Tests

- Integration tests use **build tags** and live **co-located** with the package they test (not in a separate `test/` directory).
- File naming: `*_integration_test.go` with `//go:build integration` at the top.
- `make test` runs unit tests only (`go test -race ./...`). `make test-integration` runs everything (`go test -race -tags integration ./...`).
- Use `TestMain` for expensive one-time setup/teardown (e.g., starting a test daemon, creating temp DBs).
- Integration tests should use **real dependencies** (real SQLite via `t.TempDir()`, mock ACP server as subprocess) — not mocks of mocks.
- Keep integration tests fast enough to run in CI (~30s max per package).
...
- **Svelte 5 / Frontend** keywords: svelte, component, rune, $state, $derived, $effect, $props, snippet, sveltekit, dashboard, frontend, web/
...
| Svelte 5 / Frontend     | `svelte5-best-practices`                  | `find-docs`             |
