...
func TestWorkspaceClawpatchResolverCacheDependencies(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should invalidate resolver cache when agent capability catalog appears", func(t *testing.T) {
		...
		if got, want := agentCapabilityIDsForClawpatch(
...
func TestWorkspaceClawpatchConfigClone(t *testing.T) {
	t.Parallel()
...
func agentCapabilityIDsForClawpatch(agents []aghconfig.AgentDef, name string) []string {
