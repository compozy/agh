...
func TestRunMigrationsAppliedRegistryContract(t *testing.T) {
	...
	t.Run("Should reject applied migrations ahead of the current registry", func(t *testing.T) {
		...
		migrations := contractSchemaMigrations()
	...
	t.Run("Should reject manually recorded unknown applied migrations", func(t *testing.T) {
		...
		migrations := contractSchemaMigrations()[:1]
		if err := RunMigrations(ctx, db, migrations); err != nil {
...
func contractSchemaMigrations() []Migration {
	return []Migration{
		{
			Version:    1,
			Name:       "create_contract_schema_table",
			Statements: []string{`CREATE TABLE contract_schema (id TEXT PRIMARY KEY);`},
		},
		{
			Version:    2,
			Name:       "add_contract_schema_value",
			Statements: []string{`ALTER TABLE contract_schema ADD COLUMN value TEXT;`},
		},
	}
}
