...
func TestGoalRecoveryCleanupMigrationIntegration(t *testing.T) {
	t.Run("Should preserve a v68 checkpoint and install cleanup recovery state across reopen", func(t *testing.T) {
		t.Parallel()
		...
		if err != nil {
			t.Fatalf("OpenSQLiteDatabase(v68) error = %v", err)
		}
		migrationIndex := slices.IndexFunc(globalSchemaMigrations, func(migration store.Migration) bool {
			return migration.Version == goalRecoveryCleanupMigration.Version
		})
		if migrationIndex < 0 {
			t.Fatal("Goal recovery cleanup migration missing from registry")
		}
		if err := store.RunMigrations(ctx, db, globalSchemaMigrations[:migrationIndex]); err != nil {
			t.Fatalf("RunMigrations(v68) error = %v", err)
		}
		now := store.FormatTimestamp(time.Date(2026, 7, 11, 8, 45, 0, 0, time.UTC))
		if _, err := db.ExecContext(
			ctx,
			`INSERT INTO workspaces (id, root_dir, name, created_at, updated_at)
			 VALUES ('ws-goal-v68', '/tmp/ws-goal-v68', 'Goal v68', ?, ?)`,
			now,
			now,
		); err != nil {
			t.Fatalf("insert v68 workspace error = %v", err)
		}
		if _, err := db.ExecContext(
			ctx,
			`INSERT INTO loop_runs (
				id, workspace_id, loop_name, status, last_progress_at, inputs_json, origin_kind
			) VALUES ('run-goal-v68', 'ws-goal-v68', 'delivery', 'running', ?, '{}', 'catalog')`,
			now,
		); err != nil {
			t.Fatalf("insert v68 Loop Run error = %v", err)
		}
		if _, err := db.ExecContext(
			ctx,
			`INSERT INTO loop_goal_checkpoints (
				loop_run_id, generation, node_id, item_index, control_epoch, phase, goal_status,
				turns_used, turn_limit, broken_streak, recovery_streak, prompt_attempt,
				context_state, context_nudge_ratio, updated_at
			) VALUES ('run-goal-v68', 1, 'goal', 0, 1, 'idle', 'active', 0, 3, 0, 1, 0, 'unknown', 0.8, ?)`,
			now,
		); err != nil {
			t.Fatalf("insert v68 checkpoint error = %v", err)
		}
		if err := db.Close(); err != nil {
			t.Fatalf("Close(v68) error = %v", err)
		}
		...
		assertTableHasColumn(t, reopened.db, "loop_goal_checkpoints", "compaction_baseline_used")
		assertTableHasColumn(t, reopened.db, "loop_goal_checkpoints", "compaction_recovery_required")
		assertTablesPresent(t, reopened.db, "loop_goal_session_cleanup")
		assertIndexesPresent(t, reopened.db, "loop_goal_session_cleanup", "idx_loop_goal_session_cleanup_pending")
		var recoveryStreak, recoveryRequired int
		if err := reopened.db.QueryRowContext(
			ctx,
			`SELECT recovery_streak, compaction_recovery_required
			 FROM loop_goal_checkpoints WHERE loop_run_id = 'run-goal-v68'`,
		).Scan(&recoveryStreak, &recoveryRequired); err != nil {
			t.Fatalf("read upgraded checkpoint error = %v", err)
		}
		if recoveryStreak != 1 || recoveryRequired != 0 {
			t.Fatalf("upgraded recovery state = streak:%d required:%d, want 1/0", recoveryStreak, recoveryRequired)
		}
	})
}

func TestGoalBindingReplayFencesMigrationIntegration(t *testing.T) {
	t.Run("Should preserve a v69 binding and install replay fences across reopen", func(t *testing.T) {
		t.Parallel()
		...
		migrationIndex := slices.IndexFunc(globalSchemaMigrations, func(migration store.Migration) bool {
			return migration.Version == goalBindingReplayFencesMigration.Version
		})
		if migrationIndex < 0 {
			t.Fatal("Goal binding replay fences migration missing from registry")
		}
		if err := store.RunMigrations(ctx, db, globalSchemaMigrations[:migrationIndex]); err != nil {
			t.Fatalf("RunMigrations(v69) error = %v", err)
		}
		now := store.FormatTimestamp(time.Date(2026, 7, 11, 10, 15, 0, 0, time.UTC))
		if _, err := db.ExecContext(
			ctx,
			`INSERT INTO workspaces (id, root_dir, name, created_at, updated_at)
			 VALUES ('ws-goal-v69', '/tmp/ws-goal-v69', 'Goal v69', ?, ?)`,
			now,
			now,
		); err != nil {
		...
		if _, err := db.ExecContext(
			ctx,
			`INSERT INTO loop_runs (
				id, workspace_id, loop_name, status, generation, last_progress_at, inputs_json, origin_kind
			) VALUES ('run-goal-v69', 'ws-goal-v69', 'delivery', 'running', 1, ?, '{}', 'catalog')`,
			now,
		); err != nil {
			t.Fatalf("insert v69 Loop Run error = %v", err)
		}
		if _, err := db.ExecContext(
			ctx,
			...
			) VALUES ('run-goal-v69', 'goal:v69', 1, 'attempt-v69', 'session-v69', 'ws-goal-v69',
				'profile-v69', 'policy-v69', 'creation-v69', 'origin-borrowed', 'active', ?, ?)`,
			now,
			now,
		); err != nil {
			t.Fatalf("insert v69 binding error = %v", err)
		}
		if err := db.Close(); err != nil {
			t.Fatalf("Close(v69) error = %v", err)
		}
		...
		assertTableHasColumn(t, reopened.db, "loop_session_bindings", "adopted_generation")
		assertTablesPresent(t, reopened.db, "loop_goal_binding_retry_witnesses")
		var adoptedGeneration int
		var adoptionAttemptID string
		if err := reopened.db.QueryRowContext(
			ctx,
			`SELECT adopted_generation, adoption_attempt_id FROM loop_session_bindings
			 WHERE loop_run_id = 'run-goal-v69' AND handle = 'goal:v69' AND binding_epoch = 1`,
		).Scan(&adoptedGeneration, &adoptionAttemptID); err != nil {
			t.Fatalf("read upgraded binding error = %v", err)
		}
		if adoptedGeneration != 0 || adoptionAttemptID != "attempt-v69" {
			t.Fatalf("upgraded adoption witness = %d/%q, want 0/attempt-v69", adoptedGeneration, adoptionAttemptID)
		}
	})
}
