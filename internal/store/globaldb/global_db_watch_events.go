...
func (g *GlobalDB) readLoopWatchEvents(
	...
	for rows.Next() {
		...
		if !loopWatchEventSupported(event) {
			continue
		}
		events = append(events, event)
...
func loopWatchEventSupported(event looppkg.WatchEvent) bool {
	if event.LedgerKind != loopRunEventStatusChanged {
		return true
	}
	to, ok := event.Payload["to"].(string)
	if !ok {
		to, ok = event.Payload["status"].(string)
	}
	if !ok {
		return false
	}
	return looppkg.Status(strings.TrimSpace(to)).Terminal()
}
