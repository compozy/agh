...
func TestAcpmockClawpatchRegressions(t *testing.T) {
	...
	t.Run("Should reject fixture bytes containing multiple JSON documents", func(t *testing.T) {
		valid := validClawpatchFixtureJSON("claude")
		cases := []struct {
			name string
			raw  string
		}{
	...
	t.Run("Should remove generated agent definition when post-write validation fails", func(t *testing.T) {
		...
		if err := os.WriteFile(fixturePath, []byte(validClawpatchFixtureJSON("missing-provider")), 0o600); err != nil {
...
func validClawpatchFixtureJSON(provider string) string {
