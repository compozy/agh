...
func TestProcessShutdownCancellationClawpatch(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should not start shutdown when caller context is already canceled", func(t *testing.T) {
		...
		cleanupProcessShutdownClawpatch(t, process)
...
func cleanupProcessShutdownClawpatch(t *testing.T, process *Process) {
	t.Helper()
