...
func TestInitializeBridgeRuntimeValidateClawpatch(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should reject whitespace equivalent managed bridge instance IDs", func(t *testing.T) {
		...
		runtime := InitializeBridgeRuntime{
			...
			ManagedInstances: []InitializeBridgeManagedInstance{
				managedBridgeInstanceClawpatch("brg-dup", now),
				managedBridgeInstanceClawpatch(" brg-dup ", now),
			},
		}
...
func managedBridgeInstanceClawpatch(id string, now time.Time) InitializeBridgeManagedInstance {
