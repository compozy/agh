...
func newLayoutWatchCommand(deps commandDeps) *cobra.Command {
	...
	var after uint64
	cmd := &cobra.Command{
		Use: "watch", Short: "Watch the snapshot fence and later committed topology events", Args: cobra.NoArgs,
		RunE: func(cmd *cobra.Command, _ []string) error {
			workspace, err := requiredWindowManagerFlag(workspace, windowManagerWorkspaceFlag)
			if err != nil {
				return err
			...
			return client.WatchWindowManager(cmd.Context(), workspace, afterRevision, WindowManagerStreamHandlers{
