...
type clawpatchDaemonProcess struct {
	pid     int
	done    chan struct{}
	waitErr error
}

func (p *clawpatchDaemonProcess) PID() int {
	return p.pid
}

func (p *clawpatchDaemonProcess) Done() <-chan struct{} {
	return p.done
}

func (p *clawpatchDaemonProcess) Wait() error {
	<-p.done
	return p.waitErr
}

func (p *clawpatchDaemonProcess) complete(err error) {
	p.waitErr = err
	close(p.done)
}

func TestCLIClawpatchDaemonRegressions(t *testing.T) {
	t.Parallel()

	t.Run("Should wait for the spawned daemon PID before accepting readiness", func(t *testing.T) {
		t.Parallel()

		child := &clawpatchDaemonProcess{pid: 42, done: make(chan struct{})}
		statusCalls := 0
...
func TestCLIClawpatchAuthoredBodyRegressions(t *testing.T) {
	t.Parallel()
