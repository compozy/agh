...
func (h *Handlers) createWorkspace(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, fmt.Errorf("udsapi: decode create workspace request: %w", err))
		return
	}
...
func (h *Handlers) updateWorkspace(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, fmt.Errorf("udsapi: decode update workspace request: %w", err))
		return
	}
	...
	if req.Name != nil {
		name := strings.TrimSpace(*req.Name)
		if name == "" {
			respondError(c, http.StatusBadRequest, errors.New("udsapi: name is required"))
			return
		}
		opts.Name = &name
	}
...
func (h *Handlers) resolveWorkspace(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil {
		respondError(c, http.StatusBadRequest, fmt.Errorf("udsapi: decode resolve workspace request: %w", err))
		return
	}
...
func validateCreateSessionRequest(req createSessionRequest) error {
	return apisupport.ValidateCreateSessionRequest("udsapi", req.Workspace, req.WorkspacePath)
}

func (h *Handlers) lookupWorkspaceID(ctx context.Context, ref string) (string, error) {
	return apisupport.LookupWorkspaceID(ctx, "udsapi", h.workspaces, ref)
}

func validateAbsolutePath(field string, value string) error {
	return apisupport.ValidateAbsolutePath("udsapi", field, value)
}

func validateAbsolutePaths(field string, values []string) error {
	return apisupport.ValidateAbsolutePaths("udsapi", field, values)
}
