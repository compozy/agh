...
func (o *Observer) loadSessionMetadata(ctx context.Context) ([]store.SessionInfo, error) {
	...
	for _, entry := range entries {
		...
		sessions = append(sessions, store.SessionInfo{
			...
			Lineage:          store.CloneSessionLineage(normalized.Lineage),
			State:            normalized.State,
			ACPSessionID:     normalized.ACPSessionID,
			StopReason:       stopReason,
			StopDetail:       normalized.StopDetail,
			Failure:          store.CloneSessionFailure(normalized.Failure),
			Liveness:         store.CloneSessionLivenessMeta(normalized.Liveness),
			Sandbox:          cloneSessionSandboxMeta(normalized.Sandbox),
			SoulSnapshotID:   strings.TrimSpace(normalized.SoulSnapshotID),
			SoulDigest:       strings.TrimSpace(normalized.SoulDigest),
			ParentSoulDigest: strings.TrimSpace(normalized.ParentSoulDigest),
