...
func TestBundledAghNetworkSkillContent(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "ShouldDocumentThreadIDFlag", flag: "thread-id"},
		{name: "ShouldDocumentDirectIDFlag", flag: "direct-id"},
		{name: "ShouldDocumentKindFlag", flag: "kind"},
		{name: "ShouldDocumentBodyFlag", flag: "body"},
		{name: "ShouldDocumentTargetFlag", flag: "to"},
		{name: "ShouldDocumentWorkIDFlag", flag: "work-id"},
	...
	for _, tc := range []struct {
		...
		{name: "ShouldForbidSayReceiptImitation", snippet: "Do not send `--kind say --surface direct` with `intent:\"receipt\"` or `intent:\"trace\"` as a substitute."},
		{name: "ShouldDocumentFreshWorkForSayReplies", snippet: "When replying with `--kind say --surface direct` to an inbound broadcast `say`, use the direct room's `--direct-id` and open a NEW `--work-id` only if you are starting lifecycle-bearing work."},
		{name: "ShouldForbidLifecycleKindsDirectlyAgainstSay", snippet: "Do not send `receipt` or `trace` directly against a broadcast `say`; those lifecycle kinds belong to targeted work after you open it in a direct room."},
