...
func TestBundledAghNetworkSkillContent(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "ShouldDocumentFreshWorkForSayReplies", snippet: "When replying with `--kind say --surface direct` to an inbound broadcast `say`, use the direct room's `--direct` and open a NEW `--work` only if you are starting lifecycle-bearing work."},
