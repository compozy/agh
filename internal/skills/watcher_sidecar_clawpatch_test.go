...
func TestWatcherSidecarSnapshotsClawpatch(t *testing.T) {
	t.Parallel()

	t.Run("Should keep registry seeded sidecars in the watcher baseline", func(t *testing.T) {
		t.Parallel()

		_, watcher := newWatcherWithSidecarSkillClawpatch(t)
		refreshes := 0
	...
	t.Run("Should refresh global registry after a sidecar-only change", func(t *testing.T) {
		...
		mcpPath, watcher := newWatcherWithSidecarSkillClawpatch(t)
		if err := watcher.pollOnce(context.Background()); err != nil {
...
func newWatcherWithSidecarSkillClawpatch(t *testing.T) (string, *Watcher) {
	t.Helper()
