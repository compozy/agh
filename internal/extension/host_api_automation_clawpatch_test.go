...
func TestHostAPIAutomationJobsTriggerClawpatch(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should propagate manual trigger payload to job pre fire hooks", func(t *testing.T) {
		...
		env := newHostAPITestEnv(t, withHostAPIClawpatchHooks(hooks))
		env.grant(
			"ext-automation",
...
func withHostAPIClawpatchHooks(hooks *hookspkg.Hooks) hostAPITestEnvOption {
