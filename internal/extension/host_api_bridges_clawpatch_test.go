...
func TestHostAPIHandlerBridgesMessagesIngestClawpatch(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should suppress webhook retry after request cancellation launches detached prompt", func(t *testing.T) {
		...
		driver := newClawpatchBridgePromptDriver(env.currentTime(), releasePrompt)
		broker := &recordingPromptDeliveryBroker{}
		env.useClawpatchBridgePromptDriver(t, driver, broker)
...
func (e *hostAPITestEnv) useClawpatchBridgePromptDriver(
	t *testing.T,
	...
	if err != nil {
		t.Fatalf("session.NewManager(clawpatch bridge prompt driver) error = %v", err)
	}
	...
	if err != nil {
		t.Fatalf("task.NewManager(clawpatch bridge prompt driver) error = %v", err)
	}
...
type clawpatchBridgePromptDriver struct {
	mu            sync.Mutex
	now           time.Time
	releasePrompt <-chan struct{}
	processes     map[*session.AgentProcess]*clawpatchBridgePromptProcess
	prompts       []acp.PromptRequest
	promptStarted chan struct{}
	startSeq      atomic.Int64
}

type clawpatchBridgePromptProcess struct {
	done sync.Once
	ch   chan struct{}
}

func newClawpatchBridgePromptDriver(now time.Time, releasePrompt <-chan struct{}) *clawpatchBridgePromptDriver {
	return &clawpatchBridgePromptDriver{
		now:           now,
		releasePrompt: releasePrompt,
		processes:     make(map[*session.AgentProcess]*clawpatchBridgePromptProcess),
		promptStarted: make(chan struct{}, 1),
	}
}

func (d *clawpatchBridgePromptDriver) Start(
	_ context.Context,
	opts acp.StartOpts,
) (*session.AgentProcess, error) {
	seq := d.startSeq.Add(1)
	procState := &clawpatchBridgePromptProcess{ch: make(chan struct{})}
	proc := session.NewAgentProcess(session.AgentProcessOptions{
		...
		SessionID: fmt.Sprintf("acp-clawpatch-%d", seq),
...
func (d *clawpatchBridgePromptDriver) Prompt(
	_ context.Context,
...
func (d *clawpatchBridgePromptDriver) Cancel(context.Context, *session.AgentProcess) error {
	return nil
}

func (d *clawpatchBridgePromptDriver) Stop(_ context.Context, proc *session.AgentProcess) error {
	d.mu.Lock()
...
func (d *clawpatchBridgePromptDriver) promptCount() int {
	d.mu.Lock()
	defer d.mu.Unlock()
