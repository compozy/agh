...
func TestTaskManagerHistoricalNetworkChannelIntegration(t *testing.T) {
	...
	t.Run(
		"Should start tasks against historical network channels without deriving a new coordination channel",
		func(t *testing.T) {
			t.Parallel()
	...
	t.Run(
		"Should preserve historical network channels through lease claim heartbeat and completion",
		func(t *testing.T) {
			t.Parallel()
	...
	t.Run(
		"Should preserve historical network channels through lease release reclaim and completion",
		func(t *testing.T) {
			t.Parallel()
			...
			workspaceID := registerTaskManagerWorkspace(
				t,
				db,
				"historical-channel-release-reclaim",
				filepath.Join(t.TempDir(), "workspace"),
			)
			manager := newTaskManagerIntegration(
				t,
			...
			if got, want := string(
				completed.Result,
			), `{"ok":true,"path":"historical-channel-release-reclaim"}`; got != want {
				t.Fatalf("completed.Result = %s, want %s", got, want)
			}
			if got, want := completed.NetworkChannel, "scope-direct-history"; got != want {
				t.Fatalf("completed.NetworkChannel = %q, want %q", got, want)
			...
			if got, want := eventCounts["task.run_released"], 1; got != want {
				t.Fatalf(
					"eventCounts[task.run_released] = %d, want %d (events=%#v)",
					got,
					want,
					sortedEventTypes(events),
				)
