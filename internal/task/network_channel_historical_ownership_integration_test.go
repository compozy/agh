...
func TestTaskManagerHistoricalNetworkChannelOwnershipIntegration(t *testing.T) {
	...
	t.Run("Should reject human completion and failure for token-fenced historical runs while allowing operator cancel", func(t *testing.T) {
		t.Parallel()
		...
		if got, want := eventCounts["task.run_canceled"], 1; got != want {
			t.Fatalf("eventCounts[task.run_canceled] = %d, want %d (events=%#v)", got, want, sortedEventTypes(events))
		}
