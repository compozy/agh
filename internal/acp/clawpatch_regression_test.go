...
func TestACPClawpatchRegressions(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should allow file callbacks under additional dirs advertised to the ACP agent", func(t *testing.T) {
		...
		eventsCh, err := driver.Prompt(testutil.Context(t), proc, PromptRequest{
			TurnID:  "turn-clawpatch-additional-dirs",
		...
		if !clawpatchContainsEventText(events, "from-write") {
	...
	t.Run("Should allow terminal cwd inside additional dirs advertised to the ACP agent", func(t *testing.T) {
		...
		root := t.TempDir()
		additional := t.TempDir()
		host := newClawpatchLocalToolHost(t, root, additional)
		...
		wantCwd := mustCanonicalClawpatchDir(t, additional)
		gotCwd := mustCanonicalClawpatchDir(t, output)
		if gotCwd != wantCwd {
	...
	t.Run("Should resolve terminal command from request PATH", func(t *testing.T) {
		...
		host := newClawpatchLocalToolHost(t, root)
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		defer cancel()
...
func newClawpatchLocalToolHost(t *testing.T, root string, additionalRoots ...string) *localToolHost {
	t.Helper()
...
func mustCanonicalClawpatchDir(t *testing.T, dir string) string {
	t.Helper()
...
func clawpatchContainsEventText(events []AgentEvent, want string) bool {
