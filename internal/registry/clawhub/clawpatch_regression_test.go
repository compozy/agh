...
func TestClientClawpatchRegressions(t *testing.T) {
	t.Parallel()

	t.Run("Should classify missing skill responses as registry package not found", func(t *testing.T) {
		t.Parallel()

		server := newClawpatchServer(t, func(writer http.ResponseWriter, request *http.Request) {
			...
			writeClawpatchResponse(t, writer, "{\"error\":\"missing skill\"}")
		})
	...
	t.Run("Should reject oversized search responses before decoding", func(t *testing.T) {
		...
		server := newClawpatchServer(t, func(writer http.ResponseWriter, _ *http.Request) {
			writer.Header().Set("Content-Type", "application/json")
			writeClawpatchResponse(t, writer, "["+strings.Repeat(" ", int(maxJSONResponseBytes))+"]")
		})
	...
	t.Run("Should reject oversized info responses before decoding", func(t *testing.T) {
		...
		server := newClawpatchServer(t, func(writer http.ResponseWriter, _ *http.Request) {
			...
			writeClawpatchResponse(t, writer, body)
		})
	...
	t.Run("Should stop spooling downloads after the compressed limit is crossed", func(t *testing.T) {
		...
		const limit int64 = 64
		reader := &countingClawpatchReader{
...
func newClawpatchServer(t *testing.T, handler http.HandlerFunc) *httptest.Server {
	t.Helper()
...
func writeClawpatchResponse(t *testing.T, writer io.Writer, body string) {
	t.Helper()
...
type countingClawpatchReader struct {
	reader    *strings.Reader
	readBytes int64
}

func (r *countingClawpatchReader) Read(p []byte) (int, error) {
