...
func inboundEnvelopeFromContract(envelope bridgecontract.InboundMessageEnvelope) InboundMessageEnvelope {
	converted := InboundMessageEnvelope{
		BridgeInstanceID:  envelope.BridgeInstanceID,
		Scope:             Scope(envelope.Scope),
		WorkspaceID:       envelope.WorkspaceID,
		PeerID:            envelope.PeerID,
		ThreadID:          envelope.ThreadID,
		GroupID:           envelope.GroupID,
		...
		Content:           MessageContent{Text: envelope.Content.Text},
		EventFamily:       InboundEventFamily(envelope.EventFamily),
		ReplyToText:       envelope.ReplyToText,
		ReplyToAuthorID:   envelope.ReplyToAuthorID,
		ReplyToAuthorName: envelope.ReplyToAuthorName,
		IdempotencyKey:    envelope.IdempotencyKey,
	...
	if envelope.Attachments != nil {
		converted.Attachments = make([]MessageAttachment, len(envelope.Attachments))
		for index, attachment := range envelope.Attachments {
			converted.Attachments[index] = MessageAttachment{
				ID: attachment.ID, Name: attachment.Name, MIMEType: attachment.MIMEType, URL: attachment.URL,
			}
		}
	}
	converted.ProviderMetadata = slices.Clone(envelope.ProviderMetadata)
	if envelope.Command != nil {
		converted.Command = &InboundCommand{
			Command: envelope.Command.Command, Text: envelope.Command.Text, TriggerID: envelope.Command.TriggerID,
		}
	}
	if envelope.Action != nil {
		converted.Action = &InboundAction{
	...
	if envelope.Reaction != nil {
		converted.Reaction = &InboundReaction{
	...
	if envelope.Edit != nil {
		converted.Edit = &InboundEdit{
			MessageID: envelope.Edit.MessageID, NewText: envelope.Edit.NewText,
			OriginalTimestamp: envelope.Edit.OriginalTimestamp,
			Operation:         InboundEditOperation(envelope.Edit.Operation),
		}
	}
	if envelope.Conversation != nil {
		conversation := networkConversationRefFromContract(*envelope.Conversation)
		converted.Conversation = &conversation
	}
	return converted
}
