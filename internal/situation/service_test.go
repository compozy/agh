...
func TestContextForSessionIncludesReviewerTaskBundleWithoutActiveLease(t *testing.T) {
	...
	t.Run("Should expose review-bound task context without a worker lease", func(t *testing.T) {
		...
		run := taskpkg.Run{
			...
			Status:          taskpkg.TaskRunStatusCompleted,
			SessionID:       "sess-worker",
			RunNetworkState: &taskpkg.RunNetworkState{NetworkSpec: situationLiveSpec("reviews")},
			StartedAt:       fixedTime(),
		...
		payload, err := service.ContextForSession(context.Background(), &session.Info{
			...
			WorkspaceID:          "ws-review",
			Workspace:            "/work/agh",
			NetworkParticipation: situationLiveSpec("reviews"),
...
func TestContextForSessionKeepsTaskChannelContextWhenBundleEnrichmentFails(t *testing.T) {
	...
	t.Run("Should keep active-lease task context when bundle enrichment fails", func(t *testing.T) {
		...
		run := taskpkg.Run{
			...
			RunNetworkState: &taskpkg.RunNetworkState{NetworkSpec: situationLiveSpec("coord-active")},
			QueuedAt:        fixedTime(),
			StartedAt:       fixedTime().Add(time.Minute),
		}
		service := NewService(Deps{
		...
		payload, err := service.ContextForSession(context.Background(), &session.Info{
			...
			Workspace:            "/work/agh",
			NetworkParticipation: situationLiveSpec("coord-active"),
			Type:                 session.SessionTypeSystem,
			State:                session.StateActive,
			CreatedAt:            fixedTime(),
			UpdatedAt:            fixedTime(),
	...
	t.Run("Should keep review-bound task context when bundle enrichment fails", func(t *testing.T) {
		...
		run := taskpkg.Run{
			...
			Status:          taskpkg.TaskRunStatusCompleted,
			SessionID:       "sess-worker",
			RunNetworkState: &taskpkg.RunNetworkState{NetworkSpec: situationLiveSpec("coord-run")},
			StartedAt:       fixedTime(),
			EndedAt:         fixedTime().Add(10 * time.Minute),
		}
		review := taskpkg.RunReview{
		...
		payload, err := service.ContextForSession(context.Background(), &session.Info{
			...
			WorkspaceID:          taskRecord.WorkspaceID,
			Workspace:            "/work/agh",
			NetworkParticipation: situationLiveSpec("coord-review"),
			Type:                 session.SessionTypeSystem,
			State:                session.StateActive,
			CreatedAt:            fixedTime(),
			UpdatedAt:            fixedTime(),
...
func situationLiveSpec(channelID string) participation.Spec {
