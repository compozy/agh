...
func TestBrokerSetTransportFlushesQueuedResume(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		RoutingKey:    testRoutingKey("chan-resume-route", "peer-resume-route"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-resume-route",
...
func TestBrokerProjectEventDeduplicatesAndFailsSession(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-project",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-project", "peer-project"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-project",
			PeerID:            "peer-project",
			Mode:              DeliveryModeReply,
		},
...
func TestBrokerRejectedProjectedEventDoesNotAdvanceStateOrConsumeFingerprint(t *testing.T) {
	...
	routingKey := testRoutingKey("chan-project-saturated", "peer-project-saturated")
	target := DeliveryTarget{
		ChannelInstanceID: "chan-project-saturated",
	...
	ctx := testutil.Context(t)
	if err := broker.Deliver(ctx, testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 1, DeliveryEventTypeStart, "hello", false)); err != nil {
		t.Fatalf("Deliver(regA start) error = %v", err)
	}
	waitForCalls(t, transport, 1)
	if err := broker.Deliver(ctx, testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 1, DeliveryEventTypeStart, "other", false)); err != nil {
		t.Fatalf("Deliver(regB start) error = %v", err)
	}
	if err := broker.Deliver(ctx, testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 2, DeliveryEventTypeFinal, "other done", true)); err != nil {
...
func TestBrokerRejectedFailSessionDoesNotFinalizeDelivery(t *testing.T) {
	...
	routingKey := testRoutingKey("chan-fail-saturated", "peer-fail-saturated")
	target := DeliveryTarget{
		ChannelInstanceID: "chan-fail-saturated",
...
func TestDeliveryValidationAndMetadataHelpers(t *testing.T) {
	...
	routingKey := testRoutingKey("chan-validate", "peer-validate")
	target := DeliveryTarget{
		ChannelInstanceID: "chan-validate",
...
func TestBrokerProjectEventLockedCoversTerminalAndIgnoredPaths(t *testing.T) {
	...
	delivery := &activeDelivery{
		...
		channelInstanceID: "chan-locked",
		routingKey:        testRoutingKey("chan-locked", "peer-locked"),
		target: DeliveryTarget{
			ChannelInstanceID: "chan-locked",
			PeerID:            "peer-locked",
			Mode:              DeliveryModeReply,
...
func TestBrokerEnqueueEventLockedCoversReplacementAndSaturation(t *testing.T) {
	...
	route := &routeWorker{channelInstanceID: "chan-queue"}
	delivery := &activeDelivery{deliveryID: "del-queue"}

	start := testDeliveryEvent(
		delivery.deliveryID,
		"chan-queue",
		testRoutingKey("chan-queue", "peer-queue"),
		DeliveryTarget{
			ChannelInstanceID: "chan-queue",
			PeerID:            "peer-queue",
			Mode:              DeliveryModeReply,
		},
		1,
	...
	metrics := broker.DeliveryMetrics()["chan-queue"]
