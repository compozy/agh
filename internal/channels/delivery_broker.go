...
func (b *Broker) DeliveryMetrics() map[string]ChannelDeliveryMetrics {
	if b == nil {
		return nil
	}
	...
	snapshot := make(map[string]ChannelDeliveryMetrics, len(b.metrics))
	for channelInstanceID, metrics := range b.metrics {
		if metrics == nil {
			continue
		}
		...
		snapshot[channelInstanceID] = ChannelDeliveryMetrics{
	...
	for _, route := range b.routes {
		if route == nil || route.channelInstanceID == "" {
			continue
		}
		entry := snapshot[route.channelInstanceID]
		entry.ChannelInstanceID = route.channelInstanceID
		entry.DeliveryBacklog += len(route.queue)
		snapshot[route.channelInstanceID] = entry
	}
...
func (b *Broker) RegisterPromptDelivery(ctx context.Context, reg PromptDeliveryRegistration) (*DeliverySnapshot, error) {
	if b == nil {
		return nil, errors.New("channels: delivery broker is required")
	}
	if ctx == nil {
		return nil, errors.New("channels: delivery registration context is required")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("channels: hash delivery routing key: %w", err)
	}
	...
	delivery := &activeDelivery{
		...
		channelInstanceID: normalized.RoutingKey.ChannelInstanceID,
	...
	b.ensureRouteLocked(routeHash, normalized.RoutingKey.ChannelInstanceID, normalized.ExtensionName)
	b.mu.Unlock()
...
func (b *Broker) Deliver(ctx context.Context, evt DeliveryEvent) error {
	if b == nil {
		return errors.New("channels: delivery broker is required")
	}
	if ctx == nil {
		return errors.New("channels: delivery context is required")
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	routeHash, err := normalized.RoutingKey.Hash()
	if err != nil {
		return fmt.Errorf("channels: hash delivery routing key: %w", err)
	}
	...
	if delivery.routeHash != routeHash {
		b.mu.Unlock()
		return errors.New("channels: delivery event routing key does not match registered delivery")
	}
	route := b.ensureRouteLocked(routeHash, normalized.ChannelInstanceID, delivery.extensionName)
...
func (b *Broker) Snapshot(ctx context.Context, deliveryID string) (*DeliverySnapshot, error) {
	...
	if ctx == nil {
		return nil, errors.New("channels: delivery snapshot context is required")
	}
	if err := ctx.Err(); err != nil {
		return nil, err
	}
	...
	if trimmed == "" {
		return nil, errors.New("channels: delivery snapshot id is required")
	}
...
func (b *Broker) ProjectEvent(ctx context.Context, sessionID string, event DeliveryProjectionEvent) error {
	...
	if ctx == nil {
		return errors.New("channels: delivery projection context is required")
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	route := b.ensureRouteLocked(delivery.routeHash, delivery.channelInstanceID, delivery.extensionName)
...
// terminal error so adapters do not silently orphan channel responses.
func (b *Broker) FailSession(ctx context.Context, sessionID string, reason string) error {
	if b == nil {
		return errors.New("channels: delivery broker is required")
	}
	if ctx == nil {
		return errors.New("channels: delivery fail context is required")
	}
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	for _, deliveryID := range deliveryIDs {
		...
		projected := DeliveryEvent{
			...
			ChannelInstanceID: delivery.channelInstanceID,
...
func (b *Broker) ensureRouteLocked(hash string, channelInstanceID string, extensionName string) *routeWorker {
	...
	route := &routeWorker{
		hash:              hash,
		channelInstanceID: strings.TrimSpace(channelInstanceID),
...
func (b *Broker) processQueueItem(route *routeWorker, item deliveryQueueItem) bool {
	...
	ack, err := transport.DeliverChannel(callCtx, route.extensionName, req)
	cancel()
	if err != nil {
...
func (b *Broker) handleSendFailure(route *routeWorker, deliveryID string, reason error) {
	...
	if reason != nil {
		...
		b.recordDeliveryIssueLocked(delivery.channelInstanceID, reason.Error())
	}
}
...
func (b *Broker) enqueueEventLocked(route *routeWorker, delivery *activeDelivery, event DeliveryEvent) error {
	...
	case DeliveryEventTypeStart:
		...
		if len(route.queue) >= b.queueCapacity && !b.dropQueuedDeltaLocked(route) {
			b.recordDeliveryDropLocked(route.channelInstanceID, "queue_saturated")
	...
	case DeliveryEventTypeFinal, DeliveryEventTypeError:
		...
		return nil
	default:
		return fmt.Errorf("channels: unsupported projected delivery event type %q", event.EventType)
	}
}
...
func (b *Broker) applyQueuedEventLocked(delivery *activeDelivery, event DeliveryEvent) {
	...
	if normalizedType == DeliveryEventTypeError {
		...
		b.recordDeliveryFailureLocked(delivery.channelInstanceID, delivery.errorText)
...
func (b *Broker) dropQueuedDeltaLocked(route *routeWorker) bool {
	...
	for idx, item := range route.queue {
		...
		b.recordDeliveryDropLocked(route.channelInstanceID, "coalesced")
		return true
	}
	return false
}

func (b *Broker) metricsLocked(channelInstanceID string) *instanceDeliveryMetrics {
	if b.metrics == nil {
...
func (b *Broker) recordDeliveryDropLocked(channelInstanceID string, reason string) {
	metrics := b.metricsLocked(channelInstanceID)
	if metrics == nil {
		return
	}
...
func (b *Broker) recordDeliveryIssueLocked(channelInstanceID string, message string) {
	metrics := b.metricsLocked(channelInstanceID)
	if metrics == nil {
		return
	}
...
func (b *Broker) recordDeliveryFailureLocked(channelInstanceID string, message string) {
	metrics := b.metricsLocked(channelInstanceID)
	if metrics == nil {
		return
	}
