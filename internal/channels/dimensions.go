...
// - `group_id` identifies a shared container such as a room, forum, channel, or guild.
//
...
func (m PlatformDimensionMapping) Validate() error {
	...
	if normalized.PeerIDConcept == "" && normalized.ThreadIDConcept == "" && normalized.GroupIDConcept == "" {
		return errors.New("channels: platform dimension mapping must describe at least one routing dimension")
	}
	return nil
}
...
func validateRoutingDimensions(policy RoutingPolicy, dims RoutingDimensions) error {
	...
	if normalizedPolicy.IncludePeer && normalizedDims.PeerID == "" {
		return errors.New("channels: routing policy requires peer id")
	}
	if normalizedPolicy.IncludeThread && normalizedDims.ThreadID == "" {
		return errors.New("channels: routing policy requires thread id")
	}
	if normalizedPolicy.IncludeGroup && normalizedDims.GroupID == "" {
		return errors.New("channels: routing policy requires group id")
	}
...
func validateRoutingKeyBase(instance ChannelInstance, key RoutingKey) error {
	normalizedInstance := instance.normalize()
	normalizedKey := key.normalize()

	if normalizedKey.ChannelInstanceID != "" && normalizedKey.ChannelInstanceID != normalizedInstance.ID {
		return fmt.Errorf(
			"channels: routing key channel instance id %q does not match instance %q",
			normalizedKey.ChannelInstanceID,
			normalizedInstance.ID,
		)
	}
	if normalizedKey.Scope != "" && normalizedKey.Scope != normalizedInstance.Scope {
		return fmt.Errorf(
			"channels: routing key scope %q does not match instance scope %q",
			normalizedKey.Scope,
	...
	if normalizedKey.WorkspaceID != "" && normalizedKey.WorkspaceID != normalizedInstance.WorkspaceID {
		return fmt.Errorf(
			"channels: routing key workspace id %q does not match instance workspace id %q",
