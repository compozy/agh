...
var (
	...
	ErrDeliveryNotFound = errors.New("channels: delivery not found")
	// ErrDeliveryQueueSaturated reports that a bounded delivery queue could not accept more work.
	ErrDeliveryQueueSaturated = errors.New("channels: delivery queue saturated")
	// ErrDeliveryTransportUnavailable reports that the broker has no usable extension delivery transport.
	ErrDeliveryTransportUnavailable = errors.New("channels: delivery transport unavailable")
)
...
// DeliveryTransport delivers negotiated daemon->extension channel requests.
...
type DeliveryTransport interface {
	DeliverChannel(ctx context.Context, extensionName string, req DeliveryRequest) (DeliveryAck, error)
}

// DeliveryBroker is the daemon-owned outbound delivery surface used by the
// channel runtime. Prompt projection registers live deliveries separately and
...
// to project prompt output into delivery-oriented channel events. It remains
// ACP-agnostic so `internal/channels` does not depend on runtime transport packages.
...
// `channels/deliver`. Regular streaming requests carry only Event. Recovery
...
func (r DeliveryRequest) Validate() error {
	...
	if r.Snapshot == nil {
		if normalizeDeliveryEventType(r.Event.EventType) == DeliveryEventTypeResume {
			return errors.New("channels: resume delivery request requires a snapshot")
		}
		return nil
	}
	...
	if r.Snapshot.DeliveryID != r.Event.DeliveryID {
		return errors.New("channels: delivery request snapshot must match event delivery id")
	}
	if r.Snapshot.ChannelInstanceID != r.Event.ChannelInstanceID {
		return errors.New("channels: delivery request snapshot must match event channel instance id")
	}
	return nil
}

// DeliveryAck is the negotiated extension->daemon acknowledgement payload for
// one `channels/deliver` request.
...
func (a DeliveryAck) ValidateFor(event DeliveryEvent) error {
	...
	if normalized.DeliveryID != "" && normalized.DeliveryID != strings.TrimSpace(event.DeliveryID) {
		return fmt.Errorf(
			"channels: delivery ack delivery id %q does not match event %q",
			normalized.DeliveryID,
			strings.TrimSpace(event.DeliveryID),
		)
	}
	if normalized.Seq != 0 && normalized.Seq != event.Seq {
		return fmt.Errorf("channels: delivery ack sequence %d does not match event %d", normalized.Seq, event.Seq)
	}
	return nil
}
...
func (s DeliverySnapshot) Validate() error {
	...
	if err := requireField(normalized.ChannelInstanceID, "delivery snapshot channel instance id"); err != nil {
		return err
	}
	...
	if normalized.RoutingKey.ChannelInstanceID != normalized.ChannelInstanceID {
		return errors.New("channels: delivery snapshot channel instance id must match routing key")
	}
	...
	if normalized.DeliveryTarget.ChannelInstanceID != normalized.ChannelInstanceID {
		return errors.New("channels: delivery snapshot channel instance id must match delivery target")
	}
	if normalized.LatestSeq < 0 {
		return fmt.Errorf("channels: invalid delivery snapshot latest sequence %d", normalized.LatestSeq)
	}
	if normalized.LastSentSeq < 0 {
		return fmt.Errorf("channels: invalid delivery snapshot last sent sequence %d", normalized.LastSentSeq)
	}
	if normalized.LastAckedSeq < 0 {
		return fmt.Errorf("channels: invalid delivery snapshot last acked sequence %d", normalized.LastAckedSeq)
	}
	if normalized.LastAckedSeq > normalized.LastSentSeq {
		return errors.New("channels: delivery snapshot last acked sequence cannot exceed last sent sequence")
	}
	if normalized.LastSentSeq > normalized.LatestSeq {
		return errors.New("channels: delivery snapshot last sent sequence cannot exceed latest sequence")
	}
	if normalized.UpdatedAt.IsZero() {
		return errors.New("channels: delivery snapshot updated at is required")
	}
...
// PromptDeliveryRegistration binds one session prompt turn to a routed channel
...
func (r PromptDeliveryRegistration) Validate() error {
	...
	if normalized.DeliveryTarget.ChannelInstanceID != normalized.RoutingKey.ChannelInstanceID {
		return errors.New("channels: prompt delivery registration target must match routing key channel instance")
	}
	return nil
}
...
// negotiated `channels/deliver` call.
...
func validateDeliveryEventType(value string, final bool) error {
	...
	case DeliveryEventTypeStart:
		if final {
			return errors.New("channels: delivery start event cannot be final")
		}
		return nil
	case DeliveryEventTypeDelta:
		if final {
			return errors.New("channels: delivery delta event cannot be final")
		}
		return nil
	case DeliveryEventTypeFinal:
		if !final {
			return errors.New("channels: delivery final event must set final=true")
		}
		return nil
	case DeliveryEventTypeError:
		if !final {
			return errors.New("channels: delivery error event must set final=true")
		}
		return nil
	case DeliveryEventTypeResume:
		return nil
	case "":
		return errors.New("channels: delivery event type is required")
	default:
		return fmt.Errorf("channels: unsupported delivery event type %q", strings.TrimSpace(value))
	}
}
