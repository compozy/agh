...
func (f *fakeDeliveryTransport) DeliverChannel(
	ctx context.Context,
	extensionName string,
	req DeliveryRequest,
) (DeliveryAck, error) {
...
func TestBrokerDeliversInOrderPerRoutingKeyWhileOtherRoutesStayActive(t *testing.T) {
	...
	regA := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		RoutingKey:    testRoutingKey("chan-a", "peer-a"),
	...
	regB := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-b",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-b", "peer-b"),
		DeliveryTarget: DeliveryTarget{
	...
	deliveries := []DeliveryEvent{
		testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 1, DeliveryEventTypeStart, "hello", false),
		testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 2, DeliveryEventTypeDelta, "hello again", false),
		testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 3, DeliveryEventTypeFinal, "hello again", true),
		testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 1, DeliveryEventTypeStart, "route b", false),
		testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 2, DeliveryEventTypeFinal, "route b", true),
	}
...
func TestBrokerCoalescesIntermediateDeltaUnderBackpressure(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-1",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-1", "peer-1"),
		DeliveryTarget: DeliveryTarget{
	...
	events := []DeliveryEvent{
		testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 1, DeliveryEventTypeStart, "h", false),
		testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 2, DeliveryEventTypeDelta, "he", false),
		testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 3, DeliveryEventTypeDelta, "hello", false),
		testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 4, DeliveryEventTypeFinal, "hello!", true),
	}
...
func TestBrokerAckTracksRemoteAndReplacementIDs(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-ack",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-ack", "peer-ack"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-ack",
			PeerID:            "peer-ack",
			Mode:              DeliveryModeReply,
		},
	})
	...
	ctx := testutil.Context(t)
	if err := broker.Deliver(ctx, testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 1, DeliveryEventTypeStart, "hello", false)); err != nil {
		t.Fatalf("Deliver(start) error = %v", err)
	}
	if err := broker.Deliver(ctx, testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 2, DeliveryEventTypeDelta, "hello world", false)); err != nil {
...
func TestBrokerSnapshotCapturesActiveDeliveryAfterFailure(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-resume",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-resume", "peer-resume"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-resume",
			PeerID:            "peer-resume",
			Mode:              DeliveryModeReply,
		},
	})
...
func TestBrokerDeliveryMetricsReflectBacklogAndClearAfterAck(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-metrics",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-metrics", "peer-metrics"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-metrics",
			PeerID:            "peer-metrics",
			Mode:              DeliveryModeReply,
		},
	})
	...
	if err := broker.Deliver(ctx, testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 2, DeliveryEventTypeDelta, "hello again", false)); err != nil {
		t.Fatalf("Deliver(delta) error = %v", err)
	}
	waitForCalls(t, transport, 1)

	metrics := broker.DeliveryMetrics()["chan-metrics"]
...
func TestBrokerDeliveryMetricsCaptureTerminalFailures(t *testing.T) {
	...
	reg := mustRegisterTestDelivery(t, broker, PromptDeliveryRegistration{
		...
		TurnID:        "turn-failure",
		ExtensionName: "ext-telegram",
		RoutingKey:    testRoutingKey("chan-failure", "peer-failure"),
		DeliveryTarget: DeliveryTarget{
			ChannelInstanceID: "chan-failure",
			PeerID:            "peer-failure",
			Mode:              DeliveryModeReply,
		},
	})
	...
	errorEvent := testDeliveryEvent(reg.DeliveryID, reg.ChannelInstanceID, reg.RoutingKey, reg.DeliveryTarget, 2, DeliveryEventTypeError, "boom", true)
	...
	metrics := broker.DeliveryMetrics()["chan-failure"]
...
func TestBrokerRejectedDeliverDoesNotAdvanceSnapshot(t *testing.T) {
	...
	routingKey := testRoutingKey("chan-saturated", "peer-saturated")
	target := DeliveryTarget{
		ChannelInstanceID: "chan-saturated",
	...
	ctx := testutil.Context(t)
	if err := broker.Deliver(ctx, testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 1, DeliveryEventTypeStart, "alpha", false)); err != nil {
		t.Fatalf("Deliver(regA start) error = %v", err)
	}
	waitForCalls(t, transport, 1)
	if err := broker.Deliver(ctx, testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 1, DeliveryEventTypeStart, "bravo", false)); err != nil {
		t.Fatalf("Deliver(regB start) error = %v", err)
	}
	if err := broker.Deliver(ctx, testDeliveryEvent(regB.DeliveryID, regB.ChannelInstanceID, regB.RoutingKey, regB.DeliveryTarget, 2, DeliveryEventTypeFinal, "bravo done", true)); err != nil {
		t.Fatalf("Deliver(regB final) error = %v", err)
	}

	err := broker.Deliver(ctx, testDeliveryEvent(regA.DeliveryID, regA.ChannelInstanceID, regA.RoutingKey, regA.DeliveryTarget, 2, DeliveryEventTypeFinal, "alpha done", true))
...
func testRoutingKey(channelInstanceID string, peerID string) RoutingKey {
	return RoutingKey{
