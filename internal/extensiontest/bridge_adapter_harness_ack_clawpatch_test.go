...
func TestValidateConformanceDeliveryAckTrackingClawpatch(t *testing.T) {
	t.Parallel()
	...
	t.Run("Should clear missing ack after an explicit resume delivery", func(t *testing.T) {
		...
		report.Deliveries = []DeliveryRecord{
			...
			testResumeDeliveryRecordClawpatch("delivery-1", 2),
		}
...
func testResumeDeliveryRecordClawpatch(deliveryID string, seq int64) DeliveryRecord {
