...
const (
	EnvHandshakePath = "AGH_CHANNEL_ADAPTER_HANDSHAKE_PATH"
	EnvInstancePath  = "AGH_CHANNEL_ADAPTER_INSTANCE_PATH"
	EnvStatePath     = "AGH_CHANNEL_ADAPTER_STATE_PATH"
	EnvDeliveryPath  = "AGH_CHANNEL_ADAPTER_DELIVERY_PATH"
	EnvIngestPath    = "AGH_CHANNEL_ADAPTER_INGEST_PATH"
	EnvUpdatesPath   = "AGH_CHANNEL_ADAPTER_UPDATES_PATH"
	EnvStartsPath    = "AGH_CHANNEL_ADAPTER_STARTS_PATH"
	EnvShutdownPath  = "AGH_CHANNEL_ADAPTER_SHUTDOWN_PATH"
	EnvCrashOncePath = "AGH_CHANNEL_ADAPTER_CRASH_ONCE_PATH"
)
...
// DeliveryRecord captures one `channels/deliver` request plus the adapter ack
...
// StateRecord captures one adapter-driven `channels/instances/report_state`
// result marker.
...
type ConformanceExpectation struct {
	...
	RequireResume       bool
	ExpectedFinalStatus channelspkg.ChannelStatus
}
...
// ValidateConformance checks the adapter evidence against the reusable channel
...
func ValidateConformance(report ConformanceReport, expect ConformanceExpectation) error {
	...
	if report.Handshake == nil {
		...
		if !slices.Contains(request.Methods.ExtensionServices, "channels/deliver") {
			issues = append(issues, ConformanceIssue{
				Code:    "missing_delivery_negotiation",
				Message: "initialize did not negotiate channels/deliver",
			})
		}
		if request.Runtime.Channel == nil {
			issues = append(issues, ConformanceIssue{
				Code:    "missing_channel_runtime",
				Message: "initialize runtime.channel was nil",
			})
		} else {
			if expect.InstanceID != "" && strings.TrimSpace(request.Runtime.Channel.Instance.ID) != strings.TrimSpace(expect.InstanceID) {
				issues = append(issues, ConformanceIssue{
					Code: "wrong_instance",
					Message: fmt.Sprintf(
						"initialize runtime channel instance id = %q, want %q",
						request.Runtime.Channel.Instance.ID,
						expect.InstanceID,
					),
				})
			}
			if expect.ExtensionName != "" && strings.TrimSpace(request.Runtime.Channel.Instance.ExtensionName) != strings.TrimSpace(expect.ExtensionName) {
				issues = append(issues, ConformanceIssue{
					Code: "wrong_extension",
					Message: fmt.Sprintf(
						"initialize runtime channel extension = %q, want %q",
						request.Runtime.Channel.Instance.ExtensionName,
						expect.ExtensionName,
					),
				})
			}

			bound := make(map[string]struct{}, len(request.Runtime.Channel.BoundSecrets))
			for _, secret := range request.Runtime.Channel.BoundSecrets {
	...
	if expect.RequireStateReport && len(report.States) == 0 {
		issues = append(issues, ConformanceIssue{
			...
			Message: "adapter did not report channel state",
		})
	}
	...
	for _, record := range report.Deliveries {
		...
		if eventType == channelspkg.DeliveryEventTypeResume {
			sawResume = true
		...
		if event.Seq > 1 && eventType != channelspkg.DeliveryEventTypeResume && strings.TrimSpace(record.Ack.ReplaceRemoteMessageID) == "" {
			issues = append(issues, ConformanceIssue{
...
// HarnessConfig configures one subprocess-backed channel adapter test harness.
...
// contract used to validate channel adapters end to end.
type Harness struct {
...
func NewHarness(t testing.TB, cfg HarnessConfig) *Harness {
	...
	channelRegistry := channelspkg.NewRegistry(globalDB, channelspkg.WithNow(func() time.Time { return now }))
	createReq := channelspkg.CreateInstanceRequest{
		ID:            firstNonEmpty(cfg.ChannelInstanceID, "chan-telegram-reference"),
	...
	if createReq.RoutingPolicy == (channelspkg.RoutingPolicy{}) {
		createReq.RoutingPolicy = channelspkg.RoutingPolicy{IncludePeer: true}
	}
	instance, err := channelRegistry.CreateInstance(aghtestutil.Context(t), createReq)
	if err != nil {
		t.Fatalf("channelRegistry.CreateInstance() error = %v", err)
	}
	...
	telemetrySink := &deferredChannelTelemetrySink{}
	...
	manager := extensionpkg.NewManager(
		...
		extensionpkg.WithChannelRuntimeResolver(&stubChannelRuntimeResolver{
		...
		extensionpkg.WithChannelTelemetrySink(telemetrySink),
		extensionpkg.WithHostMethodHandler("channels/messages/ingest", hostForwarder("channels/messages/ingest")),
		extensionpkg.WithHostMethodHandler("channels/instances/get", hostForwarder("channels/instances/get")),
		extensionpkg.WithHostMethodHandler("channels/instances/report_state", hostForwarder("channels/instances/report_state")),
		extensionpkg.WithHealthCheckTimeout(20*time.Millisecond),
		extensionpkg.WithSubprocessSignalGrace(15*time.Millisecond),
	)

	broker := channelspkg.NewBroker(manager, cfg.BrokerOptions...)
	observer, err := observepkg.New(
		...
		observepkg.WithChannelSource(harnessChannelSource{service: channelRegistry, broker: broker}),
	...
	notifier := extensionpkg.NewChannelDeliveryNotifier(broker, observer)
	...
	hostHandler = extensionpkg.NewHostAPIHandler(
		...
		extensionpkg.WithHostAPIChannelRegistry(channelRegistry),
		extensionpkg.WithHostAPIChannelDedupStore(globalDB),
		extensionpkg.WithHostAPIDeliveryBroker(broker),
		extensionpkg.WithHostAPINow(func() time.Time { return now }),
		extensionpkg.WithHostAPIChannelIngressConfig(15*time.Minute, time.Minute),
...
// QueryChannelHealth returns the current per-instance channel health rows.
func (h *Harness) QueryChannelHealth(t testing.TB) []observepkg.ChannelInstanceHealth {
	t.Helper()
	rows, err := h.Observer.QueryChannelHealth(aghtestutil.Context(t))
	if err != nil {
		t.Fatalf("observer.QueryChannelHealth() error = %v", err)
	}
	return rows
}
...
func (h *Harness) Report(t testing.TB) ConformanceReport {
	...
	if instance, err := readJSONFile[channelspkg.ChannelInstance](h.Markers.Instance); err == nil {
...
type harnessChannelSource struct {
	service *channelspkg.Service
	broker  *channelspkg.Broker
}

type deferredChannelTelemetrySink struct {
	observer *observepkg.Observer
}

func (s *deferredChannelTelemetrySink) RecordChannelAuthFailure(channelInstanceID string) {
	if s == nil || s.observer == nil {
		return
	}
	s.observer.RecordChannelAuthFailure(channelInstanceID)
}

func (s *deferredChannelTelemetrySink) RecordChannelRuntimeIssue(channelInstanceID string, status channelspkg.ChannelStatus, message string) {
	if s == nil || s.observer == nil {
		return
	}
	s.observer.RecordChannelRuntimeIssue(channelInstanceID, status, message)
}

func (s *deferredChannelTelemetrySink) ClearChannelRuntimeIssue(channelInstanceID string) {
	if s == nil || s.observer == nil {
		return
	}
	s.observer.ClearChannelRuntimeIssue(channelInstanceID)
}

func (s harnessChannelSource) ListInstances(ctx context.Context) ([]channelspkg.ChannelInstance, error) {
	if s.service == nil {
		return nil, nil
	}
...
func (s harnessChannelSource) ListRoutes(ctx context.Context, channelInstanceID string) ([]channelspkg.ChannelRoute, error) {
	if s.service == nil {
		return nil, nil
	}
	return s.service.ListRoutes(ctx, channelInstanceID)
}

func (s harnessChannelSource) DeliveryMetrics() map[string]channelspkg.ChannelDeliveryMetrics {
	if s.broker == nil {
		return nil
	}
...
func defaultResolvedWorkspace(root string, now time.Time) workspacepkg.ResolvedWorkspace {
	return workspacepkg.ResolvedWorkspace{
		Workspace: workspacepkg.Workspace{
			ID:           "ws-channel-adapter",
			RootDir:      root,
			Name:         "channel-adapter-workspace",
			DefaultAgent: "coder",
			CreatedAt:    now,
			UpdatedAt:    now,
		},
...
func cloneBoundSecrets(src []subprocess.InitializeChannelBoundSecret) []subprocess.InitializeChannelBoundSecret {
	if len(src) == 0 {
		return nil
	}
	return append([]subprocess.InitializeChannelBoundSecret(nil), src...)
}
