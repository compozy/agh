...
func TestHarnessIntegrationTelegramReferenceConformance(t *testing.T) {
	...
	t.Run("ready_delivery", func(t *testing.T) {
		...
		harness.WaitForStates(t, 10*time.Second, func(states []StateRecord) bool {
			return len(states) > 0 && states[len(states)-1].Status.Normalize() == channelspkg.ChannelStatusReady
		})
		...
		harness.WaitForDeliveries(t, 10*time.Second, func(records []DeliveryRecord) bool {
			return len(records) > 0 && normalizeEventType(records[len(records)-1].Request.Event.EventType) == channelspkg.DeliveryEventTypeFinal
		})
		...
		if len(rows) == 0 {
			t.Fatal("QueryChannelHealth() = empty, want channel health rows")
		}
	})
	...
	t.Run("resume_delivery", func(t *testing.T) {
		...
		if got := harness.ObserveHealth(t).Channels.StatusCounts.Ready; got != 1 {
			t.Fatalf("ObserveHealth().Channels.StatusCounts.Ready = %d, want 1", got)
		}
	})
	...
	t.Run("auth_required_health", func(t *testing.T) {
		...
		harness.WaitForHandshake(t, 10*time.Second)
		harness.WaitForStates(t, 10*time.Second, func(states []StateRecord) bool {
			return len(states) > 0 && states[len(states)-1].Status.Normalize() == channelspkg.ChannelStatusAuthRequired
		})
		...
		if got := harness.ObserveHealth(t).Channels.StatusCounts.AuthRequired; got != 1 {
			t.Fatalf("ObserveHealth().Channels.StatusCounts.AuthRequired = %d, want 1", got)
		}
	})
}
...
func TestScriptedPromptDriverPromptStopsOnContextCancellation(t *testing.T) {
	...
	case event, ok := <-events:
		if ok {
			t.Fatalf("Prompt() event after cancellation = %#v, want closed channel", event)
		}
	case <-timer.C:
		t.Fatal("Prompt() channel did not close after cancellation")
	}
}
