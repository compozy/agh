...
Use `agh bridge manifest slack --instance <id>` for Slack app setup and `agh bridge setup whatsapp|telegram|discord` for guided, write-only secret binding. The setup commands accept strict headless JSON through the global `--json` flag; supplied and existing secrets are never echoed. WhatsApp verify tokens and Telegram `--print-only` webhook secrets must be supplied or generated with the explicit one-run `--reveal-generated-secrets` JSON disclosure, so an operator-needed value never becomes irretrievable. Telegram setup otherwise registers `setWebhook` through the daemon. Every flow ends at `agh bridge verify <id>`.

`agh bridge verify <id> --json` asks the owning adapter for typed `pass|warn|fail|skipped` checks without changing instance lifecycle state; any failed check makes the command nonzero after it writes the records. `agh doctor --only bridge --json` aggregates the same checks. After enablement, `agh bridge send-test <id> --message ...` makes a real provider delivery. `agh bridge test-delivery` remains a target-resolution dry run and sends nothing. The corresponding manifest, verify, webhook-registration, and send-test operations have HTTP and UDS parity.

Credential-bearing API, OAuth, and service destinations are operator-owned adapter environment, not instance configuration. `provider_config` rejects `api_base_url`, `oauth_token_url`, `service_url`, `openid_metadata_url`, and `token_url`; use the provider's `AGH_BRIDGE_*` process variables for trusted overrides. `webhook.public_url` must be public HTTPS, and verification blocks internal/special-use addresses, proxying, and redirects before reachability is attempted.
