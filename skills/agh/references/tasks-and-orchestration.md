...
3. Claim work with the session-bound path such as agh task next --wait -o json when prompted by the runtime.
4. Keep lease/heartbeat requirements current through daemon-provided tools.
5. Complete, fail, or release only through session-bound AGH task authority.
6. Include changed files, verification commands, and residual risks in the run summary.
...
Submit exactly one typed verdict through submit_run_review for the bound request. Use daemon-provided review_id, run_id, and delivery_id.
