...
`[goals]` sets `max_turns = 20` and `context_nudge_ratio = 0.8` for new Goals. Both keys are global/workspace-overridable, agent-mutable config paths and restart-required. `max_turns` must be positive; the ratio accepts `0.0` through `1.0`, with zero preserved. Each Run pins its resolved ratio and every Goal checkpoint copies that value, so config reload or daemon restart cannot change an active Goal.
